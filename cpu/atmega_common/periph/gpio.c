...
#ifdef MODULE_PERIPH_GPIO_IRQ
static inline int8_t _int_num(const gpio_port_t *port, gpio_pin_t pin)
{
    uint8_t num;
    ...
    for (num = 0; num < GPIO_EXT_INT_NUMOF; num++) {
        if (port == ext_ints[num].port && pin == ext_ints[num].pin) {
            return num;
        }
    }
...
int gpio_cpu_init_int(const gpio_port_t *port, gpio_pin_t pin, gpio_mode_t mode, gpio_flank_t flank,
                      gpio_cb_t cb, void *arg)
{
    ...
    if (int_num < 0) {
        ...
        for (unsigned i = 0; i < ARRAY_SIZE(pcint_mapping); i++) {
            if (pin != GPIO_PIN_UNDEF &&
                pin == pcint_mapping[i].pin && port == pcint_mapping[i].port) {
                offset = i;
                break;
            }
        }
...
static inline void pcint_handler(uint8_t bank, uint8_t enabled_pcints)
{
    ...
    while (enabled_pcints > 0) {
        ...
        if (enabled_pcints & 0x1) {
            ...
            uint8_t port_value = (_SFR_MEM8(atmega_pin_addr( pin.port )));
            uint8_t pin_value = ((port_value & pin_mask) != 0);
