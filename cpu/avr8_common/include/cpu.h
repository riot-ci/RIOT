 ...
 * processing an ISR.  When nested interrupts are enabled,  it stores how deep
 * system is processing a nested interrupt.  In special, ATxmega have three
 * selectable interrupt levels for any interrupt: low, medium and high.
 *
 ...
 * performed by the pair @ref avr8_enter_isr and @ref avr8_enter_exit.
 *
 *                           int-3
 *                              ↯
 *                              +----------+
 ...
 * This is necessary because thread stack may be shared between interrupts
...
static inline void avr8_enter_isr(void)
{
     ...
     */
    if (IS_ACTIVE(CONFIG_CPU_ARCH_AVR8_NESTED_ISR)) {
        cli();
        ++GPIOR0;
        sei();
    }
