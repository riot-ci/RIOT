...
/**
 * @brief   Flag that define if platform is current using nested ISR
 */
#ifndef AVR8_NESTED_IRQ
#define AVR8_NESTED_IRQ               (0)
#endif
...
/**
 * @brief   Flags if MCU is in ISR
 */
#define AVR8_STATE_FLAG_ISR           (0x01U)
 ...
 * Contents:
 *
 * The General Purpose I/O Register 0 (GPIOR0) is used to store how deep
 * system is processing a nested interrupt.  Current ATmega only support 1
 * level or no nested interrupts.  For ATxmega there are three selectable
 * interrupt levels for any interrupt: low, medium and high.
 *
 * If the system is running outside an interrupt, GPIOR0 will have value 0.
 * When any interrupt vector is activated, GPIOR0 will have a value higher
 * than 0.  The operations are performed at @ref avr8_enter_isr and at
 * @ref avr8_enter_exit.
 *
 *                           int-3
 *                              ↯
 ...
 * +--------+                                                  +--------+
 *
 * The scheduler is allowed to switch context only when GPIOR0 is equal to 0.
 * This is necessary because thread stack is shared between nested interrupts.
 */
...
/**
 * @brief   Global variable containing the current state of the MCU UART
 *
...
static inline void avr8_enter_isr(void)
{
     ...
     * before ISR context is left by @ref avr8_exit_isr.  For ATxmega mutual
     * exclusion exists disabling global interrupt.  In case of ATmega an
     * atomic bit operation using RMW is used.
     */
