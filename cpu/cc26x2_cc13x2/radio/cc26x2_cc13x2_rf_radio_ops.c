...
ieee802154_dev_t cc26x2_cc13x2_rf_dev = {
...
static uint8_t _ack[IEEE802154_ACK_FRAME_LEN - IEEE802154_FCS_LEN];
...
static int _tx_done = 0;    /**< Transmission result, counts the number of transmissions
                                 done */
...
void cc26x2_cc13x2_rfc_isr(void) {
    ...
    if ((RFC_DBELL->RFCPEIFG & CPE_IRQ_RX_ENTRY_DONE) &&
        ...
        /* we don't want an empty payload :-) */
        ...
        /* If all failed simply drop the frame */
        else {
            entry->status = RFC_DATA_ENTRY_PENDING;
        }
...
static int _len(ieee802154_dev_t *dev)
{
    ...
    return (*(uint16_t *)&entry->data) - CC26X2_CC13X2_RF_RX_STATUS_LEN;
}
...
static void _timer_cb(void *arg, int chan)
{
    ...
    if (chan == MAC_TIMER_CHAN_ACK) {
        iolist_t iolist = {
            .iol_next = NULL,
            .iol_base = _ack,
            .iol_len = IEEE802154_ACK_FRAME_LEN,
        };

        /* send the ACK frame and wait for completion synchronously */
