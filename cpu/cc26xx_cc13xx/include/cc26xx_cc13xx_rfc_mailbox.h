 ...
 */
enum {
    RFC_CMDSTA_PENDING = 0x00,           /**< The command has not yet been parsed */
    RFC_CMDSTA_DONE = 0x01,              /**< Command successfully parsed */
    RFC_CMDSTA_ILLEGALPOINTER = 0x81,    /**< The pointer signaled in CMDR is
                                              not valid */
    RFC_CMDSTA_UNKNOWNCOMMAND = 0x82,    /**< The command number in the command
                                              structure is unknown */
    RFC_CMDSTA_UNKNOWNDIRCOMMAND = 0x83, /**< The command number for a direct
                                              command is unknown, or the command
                                              is not a direct command */
    RFC_CMDSTA_CONTEXTERROR = 0x85,      /**< An immediate or direct command was
                                              issued in a context where it is
                                              not supported */
    ...
    RFC_CMDSTA_PARERROR = 0x87,          /**< There were errors in the command
                                              parameters that are parsed on
                                              submission. */
    RFC_CMDSTA_QUEUEERROR = 0x88,        /**< An operation on a data entry queue
                                              was attempted that was not
                                              supported by the queue in its
                                              current state */
    RFC_CMDSTA_QUEUEBUSY = 0x89,        /**< An operation on a data entry was
                                             attempted while that entry was
                                             busy */
};
...
enum {
    ...
    RFC_TRIG_EXTERNAL = 10,      /**< Trigs at an external event to the radio
                                      timer */
    RFC_TRIG_PAST_BM = 0x80,     /**< Bitmask for setting past trig bit in order
                                      to trig immediately if trigger happened in
                                      the past */
};
...
enum {
    ...
    RFC_COND_STOP_ON_FALSE = 2,  /**< Run next command if this command returned
                                      True, stop if it returned False */
    RFC_COND_STOP_ON_TRUE = 3,   /**< Stop if this command returned True, run
                                      next command if it returned False */
    RFC_COND_SKIP_ON_FALSE = 4,  /**< Run next command if this command returned
                                      True, skip a number of commands if it
                                      returned False */
    RFC_COND_SKIP_ON_TRUE = 5,   /**< Skip a number of commands if this command
                                     returned True, run next command if it
                                     returned False */
};
...
enum {
    ...
    RFC_SKIPPED = 0x0003,        /**< Operation skipped due to condition in
                                      another command */
};
/**
...
enum {
    ...
    RFC_DONE_STOPPED = 0x0404,   /**< Operation stopped after CMD_STOP command */
    RFC_DONE_ABORT = 0x0405,     /**< Operation aborted by CMD_ABORT command */
...
enum {
    ...
    RFC_ERROR_CONDITION = 0x0802,  /**< Illegal condition for next operation */
    RFC_ERROR_PAR = 0x0803,        /**< Error in a command specific parameter */
    RFC_ERROR_POINTER = 0x0804,    /**< Invalid pointer to next operation */
    RFC_ERROR_CMDID = 0x0805,      /**< Next operation has a command ID that is
                                        undefined or not a radio operation
                                        command */
    RFC_ERROR_WRONG_BG = 0x0806,   /**< FG level command not compatible with
                                        running BG level command */
    RFC_ERROR_NO_SETUP = 0x0807,   /**< Operation using Rx or Tx attempted without
                                        CMD_RADIO_SETUP */
    RFC_ERROR_NO_FS = 0x0808,      /**< Operation using Rx or Tx attempted without
                                        frequency synth configured */
    ...
    RFC_ERROR_NO_RX = 0x080C,      /**< Attempted to access data from Rx when no
                                        such data was yet received */
    RFC_ERROR_PENDING = 0x080D,    /**< Command submitted in the future with
                                        another command at different level
                                        pending */
};
/** @} */
