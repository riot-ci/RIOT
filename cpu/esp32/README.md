...
```
Please take care to checkout branch ```release/v3.0``` which was used for the port. Newer versions might also work but were not tested.
...
In order to make it clear to the interface SPI_DEV (2) that we are talking about the interface that shares its bus signals with the controller for access to the flash memory, we use the identifier ```FSPI``` here. In the technical reference, this interface is misleadingly called simply SPI.
