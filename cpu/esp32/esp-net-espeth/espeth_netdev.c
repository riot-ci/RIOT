...
#include "espeth_params.h"
#include "espeth_netdev.h"
 ...
 */
static espeth_netdev_t _espeth_dev;

/* device thread stack */
static char _espeth_stack[ESPETH_STACKSIZE];
...
static esp_err_t IRAM_ATTR _eth_input_callback(void *buffer, uint16_t len, void *eb)
{
    ...
    mutex_lock(&_espeth_dev.dev_lock);

    memcpy(_espeth_dev.rx_buf, buffer, len);
    _espeth_dev.rx_len = len;
    _espeth_dev.event = SYSTEM_EVENT_ETH_RX_DONE;
    _espeth_dev.netdev.event_callback(&_espeth_dev.netdev, NETDEV_EVENT_ISR);

    mutex_unlock(&_espeth_dev.dev_lock);

    return ESP_OK;
}

#if EMAC_PHY_POWER_PIN != GPIO_UNDEF
static void _espeth_phy_power_enable_gpio(bool enable)
{
...
static int _espeth_init(netdev_t *netdev)
{
    DEBUG("%s: netdev=%p\n", __func__, netdev);

    espeth_netdev_t* dev = (espeth_netdev_t*)netdev;
    ...
    if (ret == ESP_OK && (ret = esp_eth_init(&config)) != ESP_OK) {
        LOG_ERROR("[espeth] initialization failed");
    }
    ...
    if (ret == ESP_OK && (ret = esp_eth_enable()) != ESP_OK) {
        LOG_ERROR("[espeth] enable failed");
    }
...
static int _espeth_send(netdev_t *netdev, const iolist_t *iolist)
{
...
static int _espeth_recv(netdev_t *netdev, void *buf, size_t len, void *info)
{
    ...
    if (buf && len && dev->rx_len) {
        ...
        if (dev->rx_len > len) {
            LOG_ERROR("[espeth] Not enough space in receive buffer for %d "
                      "byte\n", dev->rx_len);
            mutex_unlock(&dev->dev_lock);
            return -ENOBUFS;
        }

        /* copy received date and reset receive lenght */
...
static int _espeth_get(netdev_t *netdev, netopt_t opt, void *val, size_t max_len)
{
...
static int _espeth_set(netdev_t *netdev, netopt_t opt, const void *val, size_t max_len)
{
    DEBUG("%s: netdev=%p opt=%s val=%p len=%u\n",
          __func__, netdev, netopt2str(opt), val, max_len);
...
static void _espeth_isr(netdev_t *netdev)
{
    DEBUG("%s: netdev=%p\n", __func__, netdev);
    ...
    _espeth_dev.event = SYSTEM_EVENT_MAX; /* no event */
...
static esp_err_t IRAM_ATTR _esp_system_event_handler(void *ctx, system_event_t *event)
{
    switch(event->event_id) {
        ...
        case SYSTEM_EVENT_ETH_CONNECTED:
            ...
            _espeth_dev.link_up = true;
            if (SYSTEM_EVENT_MAX) {
                _espeth_dev.event = SYSTEM_EVENT_ETH_CONNECTED;
                _espeth_dev.netdev.event_callback(&_espeth_dev.netdev, NETDEV_EVENT_ISR);
            }
            break;
        case SYSTEM_EVENT_ETH_DISCONNECTED:
            DEBUG("%s: Ethernet link up\n", __func__);
            _espeth_dev.link_up = false;
            if (SYSTEM_EVENT_MAX) {
                _espeth_dev.event = SYSTEM_EVENT_ETH_DISCONNECTED;
                _espeth_dev.netdev.event_callback(&_espeth_dev.netdev, NETDEV_EVENT_ISR);
            }
            break;
        default:
...
static const netdev_driver_t _espeth_driver =
{
    .send = _espeth_send,
    .recv = _espeth_recv,
    .init = _espeth_init,
...
void auto_init_esp_net_espeth (void)
{
    DEBUG("%s: initializing ESP32 Ethernet device\n", __func__);

    /* initialize locking */
    mutex_init(&_espeth_dev.dev_lock);

    /* init esp system event loop */
    esp_system_event_add_handler(_esp_system_event_handler, NULL);

    /* set the netdev driver */
    _espeth_dev.netdev.driver = &_espeth_driver;

    /* initialize netdev data structure */
    _espeth_dev.event = SYSTEM_EVENT_MAX; /* no event */
    _espeth_dev.link_up = false;
    _espeth_dev.rx_len = 0;
    _espeth_dev.tx_len = 0;
