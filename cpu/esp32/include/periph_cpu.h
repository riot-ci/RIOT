 ...
 *   about the interface that shares its bus signals with the controller for
 *   access to the flash memory, we use the identifier FSPI here. In the
 *   technical reference, this interface is misleadingly called simply SPI.
 * - Since the FSPI interface SPI_DEV(2) shares its bus signals with flash
 *   memory interface and other external memories, you can only use SPI_DEV(2)
 *   to attach external memory with same SPI mode and same bus speed but with
 *   a different CS.
...
#define SPI2_SCK    GPIO6   /* direct I/O pin FSPICLK labeled CLK */
#define SPI2_MISO   GPIO7   /* direct I/O pin FSPIQ   labeled DAT0 or SD0 */
#define SPI2_MOSI   GPIO8   /* direct I/O pin FSPID   labeled DAT1 or SD1 */
#define SPI2_CS0    GPIO11  /* direct I/O pin FSPICS0 labeled CMD, MUST be changed */
#endif
