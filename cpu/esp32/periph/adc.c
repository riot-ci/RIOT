...
int adc_init(adc_t line)
{
    ...
    if (_gpio_pin_usage[_adc_hw[rtcio].gpio] != _GPIO) {
        LOG_ERROR("GPIO%d is used for %s and cannot be used as ADC input\n",
                  _adc_hw[rtcio].gpio,
                  _gpio_pin_usage_str[_gpio_pin_usage[_adc_hw[rtcio].gpio]]);
        return -1;
    }
    ...
    /* set pin usage type  */
    _gpio_pin_usage[_adc_hw[rtcio].gpio] = _ADC;
...
int adc_vref_to_gpio25 (void)
{
    ...
    if (line == ADC_UNDEF) {
        LOG_ERROR("Have no ADC line for GPIO25\n");
        return -1;
    }
    ...
    if (adc_init(line) == 0)
    {
        ...
        SENS.sar_meas_start2.sar2_en_pad = (1 << _adc_hw[rtcio].adc_channel);
        LOG_INFO("You can now measure Vref at GPIO25\n");
        return 0;
    }
    else {
        LOG_ERROR("Could not init GPIO25 as Vref output\n");
        return -1;
    }
}
...
static bool _adc_conf_check(void)
{
    for (unsigned i = 0; i < ADC_NUMOF; i++) {
        if (_gpio_rtcio_map[ADC_PINS[i]] == RTCIO_NA) {
            LOG_ERROR("GPIO%d cannot be used as ADC line\n", ADC_PINS[i]);
            return false;
        }
    }
...
int8_t dac_init (dac_t line)
{
    ...
    if (_gpio_pin_usage[_adc_hw[rtcio].gpio] != _GPIO) {
        LOG_ERROR("GPIO%d is used for %s and cannot be used as DAC output\n",
                  _adc_hw[rtcio].gpio,
                  _gpio_pin_usage_str[_gpio_pin_usage[_adc_hw[rtcio].gpio]]);
        return DAC_NOLINE;
    }
    ...
    /* set pin usage type  */
    _gpio_pin_usage[_adc_hw[rtcio].gpio] = _DAC;
...
static bool _dac_conf_check(void)
{
    for (unsigned i = 0; i < DAC_NUMOF; i++) {
        if (_gpio_rtcio_map[DAC_PINS[i]] != RTCIO_DAC1 &&
            ...
            LOG_ERROR("GPIO%d cannot be used as DAC line\n", DAC_PINS[i]);
            return false;
        }
    }
...
int rtcio_config_sleep_mode (gpio_t pin, bool mode, bool input)
{
    ...
    switch (rtcio) {
        ...
        case RTCIO_DAC2: /* GPIO26, RTC7 */
                ...
                break;
        default:
                LOG_ERROR("GPIO %d is not an RTCIO pin and cannot be used in "
                          "sleep mode\n", pin);
                return -1;
    }
    return 0;
