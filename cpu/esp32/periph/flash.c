...
void spi_flash_drive_init (void)
{
     ...
     */
    #if CONFIG_SPI_FLASH_DRIVE_START
    if (part_top > CONFIG_SPI_FLASH_DRIVE_START) {
        LOG_ERROR("[spi_flash_drive] configured start address is to less\n");
    }
    ...
    LOG_INFO("[spi_flash_drive] flash mtd starts at address %08x\n", part_top);
