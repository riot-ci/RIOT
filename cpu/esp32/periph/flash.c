...
#define ESP_PART_TABLE_ADDR         ESP_PARTITION_TABLE_ADDR
...
void spi_flash_drive_init (void)
{
    ...
    while (part_read && part_addr < ESP_PART_TABLE_ADDR + ESP_PART_TABLE_SIZE) {
        ...
        if (part->magic == ESP_PART_ENTRY_MAGIC) {
            DEBUG("%s partition @%08x size=%08lx label=%s\n", __func__,
    ...
    /* map the partition top address to next higher multiple of 0x10000 */
    part_top = (part_top + 0x10000) & ~0xffff;
    DEBUG("%s flash mtd begins at %08lx\n", __func__, part_top);
...
{
    DEBUG("%s addr=%08lx size=%lu buf=%p\n", __func__, addr, size, buff);
