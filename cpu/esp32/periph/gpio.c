...
{
    "GPIO", "ADC", "DAC", "I2C", "PWM", "SPI", "SPI Flash", "UART", "N/A"
};
...
int gpio_init(gpio_t pin, gpio_mode_t mode)
{
    ...
    if (_gpio_pin_usage[pin] != _GPIO) {
        LOG_ERROR("GPIO%d is already used as %s signal\n", pin,
    ...
    if (pin > GPIO33) {
        switch (mode) {
            ...
            case GPIO_IN_OD_PU:
                ...
                LOG_ERROR("GPIO%d can only be used as input\n", pin);
                return -1;

            case GPIO_IN_PD:
            case GPIO_IN_PU:
                /* GPIOs 34 ... 39 have no software controlable pullups/pulldowns */
                LOG_ERROR("GPIO%d has no pullups/pulldowns\n", pin);
                return -1;
            default: break;
        }
    }
