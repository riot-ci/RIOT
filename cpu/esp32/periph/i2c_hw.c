...
{
    DEBUG ("%s dev=%lu addr=%02x reg=%02x data=%p len=%d\n", __func__,
...
static bool _i2c_init_pins(i2c_t dev)
{
    ...
    if (_gpio_pin_usage[_i2c_hw[dev].pin_scl] == _I2C) {
        _gpio_pin_usage[_i2c_hw[dev].pin_scl] = _GPIO;
    }
    if (_gpio_pin_usage[_i2c_hw[dev].pin_sda] == _I2C) {
        _gpio_pin_usage[_i2c_hw[dev].pin_sda] = _GPIO;
    }
    ...
    _gpio_pin_usage[_i2c_hw[dev].pin_scl] = _I2C;
    _gpio_pin_usage[_i2c_hw[dev].pin_sda] = _I2C;
