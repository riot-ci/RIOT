...
int i2c_init_master(i2c_t dev, i2c_speed_t speed)
{
    if (I2C_NUMOF != sizeof(_i2c_bus)/sizeof(_i2c_bus_t)) {
        LOG_TAG_INFO("i2c", "I2C_NUMOF does not match number of the "
                     "I2C_SDA_x/I2C_SCL_x definitions\n");
        LOG_TAG_INFO("i2c",
                     "Please check your configuration in file board.h\n");
    ...
    if (gpio_get_pin_usage(_i2c_bus[dev].pin_scl) == _I2C) {
        gpio_set_pin_usage(_i2c_bus[dev].pin_scl, _GPIO);
    }
    if (gpio_get_pin_usage(_i2c_bus[dev].pin_sda) == _I2C) {
        gpio_set_pin_usage(_i2c_bus[dev].pin_sda, _GPIO);
    }
    ...
    gpio_set_pin_usage(_i2c_bus[dev].pin_scl, _I2C);
    gpio_set_pin_usage(_i2c_bus[dev].pin_sda, _I2C);
...
void i2c_print_config(void)
{
    ...
    for (unsigned bus = 0; bus < I2C_NUMOF; bus++) {
        LOG_TAG_INFO("i2c", "I2C_DEV(%d) scl=%d sda=%d\n",
