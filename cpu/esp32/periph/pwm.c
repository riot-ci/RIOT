...
uint32_t pwm_init(pwm_t pwm, pwm_mode_t mode, uint32_t freq, uint16_t res)
{
    ...
    if (_pwm_hw[pwm].gpio_num == 0) {
        LOG_ERROR("PWM device %d has no assigned pins\n", pwm);
        return 0;
    }
    ...
    for (int i = 0; i < _pwm_dev[pwm].chn_num; i++) {
        ...
        if (_gpio_pin_usage[_pwm_hw[pwm].gpios[i]] == _PWM) {
            _gpio_pin_usage[_pwm_hw[pwm].gpios[i]] = _GPIO;
        }

       if (_gpio_pin_usage[_pwm_hw[pwm].gpios[i]] != _GPIO) {
            LOG_ERROR("GPIO%d is used for %s and cannot be used as PWM output\n", i,
                      _gpio_pin_usage_str[_gpio_pin_usage[_pwm_hw[pwm].gpios[i]]]);
            return 0;
        }
        ...
        _gpio_pin_usage[_pwm_hw[pwm].gpios[i]] = _PWM;
...
static bool _pwm_configuration(void)
{
    if (PWM_NUMOF > PWM_NUMOF_MAX) {
        LOG_ERROR("%d PWM devices were defined, only %d PWM are "
                  "supported\n", __func__, PWM_NUMOF, PWM_NUMOF_MAX);
        return false;
    }

    for (unsigned i = 0; i < PWM_NUMOF; i++) {
        if (_pwm_hw[i].gpio_num > PWM_CHANNEL_NUM_DEV_MAX) {
            LOG_ERROR("Number of PWM channels of device % d is %d, "
                      "at maximum only %d channels per PWM device %d are "
                      "supported\n", __func__,
                ...
                for (unsigned k = 0; k < _pwm_hw[i].gpio_num >> 2; k++) {
                    for (unsigned l = 0; l < _pwm_hw[i].gpio_num >> 2; l++) {
                        if (_pwm_hw[i].gpios[k] == _pwm_hw[j].gpios[l]) {
                            LOG_ERROR("GPIO%d is used multiple times in "
                                      "PWM devices %d and %d\n", __func__,
...
void pwm_print_config(void)
{
    ...
    for (unsigned pwm = 0; pwm < PWM_NUMOF; pwm++) {
        LOG_INFO("PWM_DEV(%d): channels=[ ", pwm);
        for (int i = 0; i < _pwm_hw[pwm].gpio_num; i++) {
            LOG_INFO("%d ", _pwm_hw[pwm].gpios[i]);
        }
        LOG_INFO("]\n");
    }
    #else
