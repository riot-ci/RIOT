...
static void IRAM_ATTR _rtt_cb(void *arg)
{
    ...
    if (_rtt_alarm_wakeup) {
        ...
        uint64_t _sys_alarm = (((uint64_t)TIMER_SYSTEM.alarm_high << 32) |
                              TIMER_SYSTEM.alarm_low) + _sys_time_offset;
        alarm = ((_sys_alarm << 15) / US_PER_SEC) + _rtt_offset;

        DEBUG("%s alarm=%u rtt_alarm=%u @rtt=%u "
              "sys_alarm=%llu @sys_time=%llu\n", __func__, alarm,
              rtt_counter.alarm, rtt_get_counter(),
              _sys_alarm, system_get_time_64());
    }
