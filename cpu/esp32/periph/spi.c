...
static const struct spi_hw_t _spi[SPI_BUS_NUM] = {
    {
        .regs = &SPI2, /* registers of controller SPI2 (HSPI) */
        .mod = PERIPH_HSPI_MODULE,
        .int_src = ETS_SPI2_INTR_SOURCE,
        .pin_sck  = SPI0_SCK,
        .pin_mosi = SPI0_MOSI,
        .pin_miso = SPI0_MISO,
        .pin_cs   = SPI0_CS0,
        .signal_sck  = HSPICLK_OUT_IDX,
        .signal_mosi = HSPID_OUT_IDX,
        .signal_miso = HSPIQ_IN_IDX
    },
    {
        .regs = &SPI3, /* registers of controller (VSPI) */
        .mod = PERIPH_VSPI_MODULE,
        .int_src = ETS_SPI3_INTR_SOURCE,
        .pin_sck  = SPI1_SCK,
        .pin_mosi = SPI1_MOSI,
        .pin_miso = SPI1_MISO,
        .pin_cs = SPI1_CS0,
        .signal_sck  = VSPICLK_OUT_IDX,
        .signal_mosi = VSPID_OUT_IDX,
        .signal_miso = VSPIQ_IN_IDX
    },
    {
        .regs = &SPI1, /* registers of controller SPI1 (FSPI) */
        .mod = PERIPH_SPI_MODULE,
        .int_src = ETS_SPI1_INTR_SOURCE,
        .pin_sck  = SPI2_SCK,
        .pin_mosi = SPI2_MOSI,
        .pin_miso = SPI2_MISO,
        .pin_cs   = SPI2_CS0,
        .signal_sck  = SPICLK_OUT_IDX,
        .signal_mosi = SPID_OUT_IDX,
        .signal_miso = SPIQ_IN_IDX
    }
};

/* allocate a mutex for each possible SPI interface */
...
#define CHECK_SPI_DEV(bus) { \
    ...
    if (_spi[bus].pin_sck == GPIO_UNDEF) { \
        LOG_ERROR("SPI_DEV(%s) is declared as not available\n", bus); \
        return; \
    } \
}
...
static void _spi_init_internal (spi_t bus)
{
    ...
    if (_gpio_pin_usage[_spi[bus].pin_sck] != _SPI &&
        _gpio_pin_usage[_spi[bus].pin_miso] != _SPI &&
        _gpio_pin_usage[_spi[bus].pin_mosi] != _SPI &&
        _gpio_pin_usage[_spi[bus].pin_cs] != _SPI) {
        return;
    }
...
void spi_init_pins(spi_t bus)
{
    ...
    if (bus != SPI_DEV(2)) {
        ...
        if (gpio_init (_spi[bus].pin_sck, GPIO_OUT) ||
            ...
            LOG_ERROR("SPI_DEV(%s) pins could not be initialized\n", bus);
            return;
        }
        ...
        _gpio_pin_usage[_spi[bus].pin_sck] = _SPI;
        _gpio_pin_usage[_spi[bus].pin_mosi] = _SPI;
        _gpio_pin_usage[_spi[bus].pin_miso] = _SPI;
...
int IRAM_ATTR spi_acquire(spi_t bus, spi_cs_t cs, spi_mode_t mode, spi_clk_t clk)
{
    ...
    DEBUG("%s bus %d: SPI_CLOCK_REG=%08lx\n",
...
static const char* _spi_names[] = { "VSPI", "HSPI", "FSPI" };

void spi_print_config(void)
{
    for (unsigned bus = 0; bus < SPI_NUMOF; bus++) {
        LOG_INFO("SPI_DEV(%d) / %s: ", bus, _spi_names[bus]);
        if (_spi[bus].pin_sck == GPIO_UNDEF) {
           LOG_INFO("not available\n");
        }
        else {
           LOG_INFO("sck=%d ", _spi[bus].pin_sck);
           LOG_INFO("miso=%d ", _spi[bus].pin_miso);
           LOG_INFO("mosi=%d ", _spi[bus].pin_mosi);
           LOG_INFO("cs=%d\n", _spi[bus].pin_cs);
        }
    }
}
