...
NORETURN void IRAM call_start_cpu0 (void)
{
    ...
    /* reset from panic handler we can by RWDT or TG0WDT */
    ...
    #ifndef ESP_IDF_HEAP_USED
    ets_printf("_heap_start %p\n", &_sheap);
    ets_printf("_heap_end   %p\n", &_eheap);
    ets_printf("_heap_free  %lu\n", get_free_heap_size());
...
static void do_global_ctors(void)
{
    /* TODO ./dist/tools/cppcheck/check.sh fails
