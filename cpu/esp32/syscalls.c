...
{
    char tmp;
    if (uart_stdio_read(&tmp, 1) > 0) {
        return tmp;
    }
    return -EOF;
}
#endif /* MODULE_UART_STDIO */
...
/* this should not happen when ESP_IDF_HEAP_USED is activated since heap_caps
...
void _abort(void)
{
    LOG_INFO("#! abort called: powering off\n");
    pm_off();
    while(1);
}
...
uint64_t system_get_time_64 (void)
{
    ...
    tmp[0] = TIMER_SYSTEM.cnt_high;
    tmp[1] = TIMER_SYSTEM.cnt_low;
    return ret;
}
...
void system_wdt_init (void)
{
    ...
    DEBUG("%s TIMERG0 wdt_config0=%08lx wdt_config1=%08lx wdt_config2=%08lx\n",
          __func__, TIMERG0.wdt_config0, TIMERG0.wdt_config1, TIMERG0.wdt_config2);
