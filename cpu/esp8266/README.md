...
3. [Toolchain Installation](#esp8266_toolchain_installation)
...
4. [Application-Specific Configurations](#esp8266_application_specific_configurations)
...
7. [Preconfigured Devices](#esp8266_preconfigured_devices)
    1. [Network Devices](#esp8266_network_devices)
        1. [Using MRF24J40 (module ```mrf24j40```)](#esp8266_using_mrf24j40)
        2. [Using ENC28J60 (module ```enc28j60```)](#esp8266_using_enc28j60)
...
# <a name="esp8266_overview"> Overview </a>
...
# <a name=esp8266_mcu_esp8266> MCU ESP8266 </a>
...
# <a name="esp8266_toolchain_installation"> Toolchain Installation</a>
...
You have following options to install the Toolchain:

- **riotdocker** image, see section [RIOT docker Toolchain (riotdocker)](#esp8266_riot_docker_toolchain)
- **precompiled toolchain** installatioI2n from GIT, see section [Precompiled Toolchain](#esp8266_toolchain_installation)
- **manual installation**, see section [Manual Toolchain Installation](#esp8266_manual_toolchain_installation)

## <a name="esp8266_riot_docker_toolchain"> RIOT docker Toolchain (riotdocker) </a>

The easiest way for toolchain installation is to use docker.

### <a name="esp8266_generating_docker_image"> Generating a riotdocker Image </a>

A ```riotdocker``` fork that only installs the ```RIOT-Xtensa-ESP8266-toolchain``` is also available at [GitHub](https://github.com/gschorcht/riotdocker-Xtensa-ESP.git). After cloning this git repository, you can use branch ```esp8266_only``` to generate a docker image with a size of "only" 990 MByte:
...
Once a Docker image has been created, it can be started with the following command:
```
...
### <a name="esp8266_using_existing_docker_image"> Using an Existing riotdocker Image </a>

Alternatively, an existing docker image from Docker Hub can be used. You can either start the [schorcht/riotbuild_esp8266](https://hub.docker.com/r/schorcht/riotbuild_esp8266) docker image which only contains the ```RIOT-Xtensa-ESP8266-toolchain``` using
```
...
## <a name="esp8266_precompiled_toolchain"> Precompiled Toolchain </a>
...
## <a name="esp8266_manual_toolchain_installation"> Manual Toolchain Installation </a>
...
### <a name="esp8266_installation_of_esp_open_sdk"> Installation of esp-open-sdk </a>
...
### <a name="esp8266_installation_of_newlib-c"> Installation of newlib-c </a>
...
### <a name="esp8266_installation_of_espressif_original_sdk"> Installation of Espressif original SDK (optional) </a>
...
# <a name="esp8266_application_specific_configurations"> Application-Specific Configurations </a>

Parameters used for peripheral devices or for device driver modules, such as GPIO pins, bus interfaces or bus speeds are typically defined in the board-specific configurations ```board.h``` and ```periph_conf.h``` or in the driver parameter configuration ```< driver>_params.h```. Most of these parameter definitions are enclosed by
```
#ifndef PARAMETER
...
#endif
...
## <a name="esp8266_application_specific_board_configuration"> Application-Specific Board Configuration </a>
...
## <a name="esp8266_application_specific_driver_configuration"> Application-Specific Driver Configuration </a>
...
# <a name="esp8266_flashing_the_device"> Flashing the Device </a>

## <a name="esp8266_toolchain_usage"> Toolchain Usage </a>
...
## <a name="esp8266_compile_options"> Compile Options </a>

The compilation process can be controlled by a number of variables in the make command:
...
QEMU | 0, 1 | 0 | Use QEMU mode and generate an image for QEMU, see section [QEMU Mode and GDB](#esp8266_qemu_mode_and_gdb).
USE_SDK | 0, 1 | 0 | Compile the SDK version (```USE_SDK=1```), see section [SDK Task Handling](#esp8266_sdk_task_handling)
USE_SPIFFS | 0, 1 | 0 | Enable the SPIFFS file system, see section [SPIFFS Device](#esp8266_spiffs_device)
USE_SW_TIMER | 0, 1 | 0 | Enable software timer implementation, only available in conjunction with ```USE_SDK=1```, see section [Timers](#esp8266_timers)

</center><br>

## <a name="esp8266_flash_modes"> Flash Modes </a>
...
# <a name="esp8266_peripherals"> Peripherals </a>

## <a name="esp8266_gpio_pins"> GPIO pins </a>
...
## <a name="esp8266_adc_channels"> ADC Channels </a>
...
## <a name="esp8266_spi_interfaces"> SPI Interfaces </a>
...
## <a name="esp8266_i2c_interfaces"> I2C Interfaces </a>
...
## <a name="esp8266_pwm_channels"> PWM Channels </a>
...
## <a name="esp8266_timers"> Timers </a>

By default, the **hardware timer implementation** is used. In this implementation there is only **one timer device** with only **one channel** available.

If you use the SDK version of the RIOT port (```USE_SDK=1```), you can activate the **software timer implementation** (```USE_SW_TIMER=1```), which implements **one timer device** with **10 channels**. The software timer uses SDK software timers to implement various timer channels. Although these SDK timers usually have a precision of a few microseconds, they can deviate up to 500 microseconds. So if you need a timer with high accuracy, you'll need to use the hardware timer with only one timer channel.

## <a name="esp8266_spiffs_device"> SPIFFS Device </a>
...
## <a name="esp8266_other_peripherals"> Other Peripherals </a>
...
# <a name="esp8266_preconfigured_devices"> Preconfigured Devices </a>

The ESP8266 port of RIOT has been tested with several common external devices that can be connected to ESP8266 boards and is preconfigured accordingly.

## <a name="esp8266_network_devices"> Network Devices </a>
...
### <a name="esp8266_using_mrf24j40"> Using MRF24J40 (module ```mrf24j40```) </a>
...
### <a name="esp8266_using_enc28j60"> Using ENC28J60 (module ```enc28j60```) </a>
...
## <a name="esp8266_sd_card_device"> SD-Card Device </a>

ESP8266 port of RIOT is preconfigured for RIOT applications that use the [SPI SD-Card driver](http://riot-os.org/api/group__drivers__sdcard__spi.html). To use SPI SD-Card driver, the ```mrf24j40``` module has to be added to a makefile:
...
# <a name="esp8266_sdk_task_handling"> SDK Task Handling </a>
...
# <a name="esp8266_qemu_mode_and_gdb"> QEMU Mode and GDB </a>
