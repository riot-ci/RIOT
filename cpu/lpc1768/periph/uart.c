...
static inline void init_base(uart_t uart, uint32_t baudrate)
{
    ...
    *(&LPC_PINCON->PINSEL0 + cfg->pinsel) &= ~(0xF << (cfg->pinsel_shift));
    /* Select uart TX mux */
    *(&LPC_PINCON->PINSEL0 + cfg->pinsel) |= (cfg->pinsel_af << (cfg->pinsel_shift));
    /* Select uart RX mux */
    *(&LPC_PINCON->PINSEL0 + cfg->pinsel) |= (cfg->pinsel_af << (cfg->pinsel_shift + 2));
...
void uart_write(uart_t uart, const uint8_t *data, size_t len)
{
    ...
    for (size_t i = 0; i < len; i++) {
        while (!(dev(uart)->LSR & (1 << 5))) {}  /* wait for THRE bit to be set */
        dev(uart)->THR = data[i];
    }
}
