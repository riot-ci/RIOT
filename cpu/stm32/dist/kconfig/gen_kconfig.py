...
def generate_kconfig(kconfig, context, overwrite, quiet):
    ...
    if (not os.path.exists(kconfig_file) or
            os.path.exists(kconfig_file) and overwrite is True):
...
def main(args):
    ...
    for kconfig in ["lines", "models"]:
        generate_kconfig(kconfig, context, args.overwrite, args.quiet)
