...
static inline void set_bit_timing(candev_stm32_t *dev)
{
    ...
    dev->conf->can->BTR = (((uint32_t)(dev->candev.bittiming.sjw - 1) << 24) & CAN_BTR_SJW) |
                          ...
                          (((uint32_t)((dev->candev.bittiming.phase_seg1 + dev->candev.bittiming.prop_seg)
                                        - 1) << 16) & CAN_BTR_TS1) |
...
static int _get(candev_t *candev, canopt_t opt, void *value, size_t max_len)
{
    ...
    case CANOPT_RX_FILTERS:
        if (max_len % sizeof(struct can_filter) != 0) {
            ...
            int en_filt = 0;
            int base_filter = get_first_filter(dev);
            int last_filter = base_filter + get_nb_filter(dev);
            for (int i = base_filter; i < last_filter; i++) {
                if ((get_master(dev)->FA1R & (1 << i)) == (1 << i)) {
                    en_filt++;
                }
            }
            if (max_len / sizeof(struct can_filter) < en_filt) {
                ...
                for (int i = 0; i < en_filt; i++) {
                    struct can_filter *filter = (struct can_filter *)value + i;
                    get_can_filter(get_master(dev), i + base_filter, &filter->can_id, &filter->can_mask);
                }
...
static int _set_filter(candev_t *candev, const struct can_filter *filter)
{
    ...
    for (i = first_filter; i < last_filter; i++) {
        if (!filter_is_set(get_master(dev), i)) {
            ...
            set_filter(get_master(dev), filter->can_id, filter->can_mask, i, i % CAN_STM32_RX_MAILBOXES);
            get_master(dev)->FMR &= ~CAN_FMR_FINIT;
            break;
        }
    }
...
static int _remove_filter(candev_t *candev, const struct can_filter *filter)
{
    ...
    for (i = first_filter; i < last_filter; i++) {
        if (filter_is_set(get_master(dev), i)) {
            ...
            if ((filt == filter->can_id) &&
                    (((filt & CAN_EFF_FLAG) && (mask == (filter->can_mask & CAN_EFF_MASK)))
                    || (!(filt & CAN_EFF_FLAG) && (mask == (filter->can_mask & CAN_SFF_MASK))))) {
                get_master(dev)->FMR = CAN_FMR_FINIT;
