...
/**
 * @brief   Mask MSB containing parity depending on data length
 */
...
#if defined(USART_CR1_M1)
    if ((databits == UART_INVALID_MODE) || (databits == UART_DATABITS_6 && !parity))
#else
    if ((databits == UART_INVALID_MODE) || (databits == UART_DATABITS_7 && !parity))
#endif
        return UART_NOMODE;

    if (parity == UART_INVALID_MODE)
        return UART_NOMODE;
