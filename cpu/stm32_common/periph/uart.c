              ...
              uart_stop_bits_t stop_bits)
{
    ...
    if (parity) {
        switch (data_bits) {
            case UART_DATA_BITS_6: data_bits = UART_DATA_BITS_7; break;
            case UART_DATA_BITS_7: data_bits = UART_DATA_BITS_8; break;
            case UART_DATA_BITS_8: data_bits = USART_CR1_M; break;
            default:
                return UART_NOMODE;
        }
    }
...
static uint8_t _data_mask(uart_t uart)
{
    ...
    if ((dev(uart)->CR1 & (USART_CR1_PCE | USART_CR1_M)) == (USART_CR1_PCE | USART_CR1_M1)) {
        return 0x3F;
    }
#endif
    if ((dev(uart)->CR1 & (USART_CR1_PCE | USART_CR1_M)) == USART_CR1_PCE) {
        return 0x7F;
    }
    return 0xFF;
}
...
static inline void irq_handler(uart_t uart)
{
    ...
    if (status & USART_ISR_RXNE) {
        isr_ctx[uart].rx_cb(isr_ctx[uart].arg, (uint8_t)dev(uart)->RDR & _data_mask(uart));
    }
    ...
    if (status & USART_SR_RXNE) {
        isr_ctx[uart].rx_cb(isr_ctx[uart].arg, (uint8_t)dev(uart)->DR & _data_mask(uart));
    }
