 ...
 */
static inline GPIO_TypeDef *_port(const gpio_port_t *port)
{
 ...
 * numbers.
 */
static inline int _port_num(const gpio_port_t *port)
{
...
void gpio_init_analog(gpio_t gpio)
{
    ...
    _port(gpio.port)->CR[gpio.pin >= 8] &= ~(0xfl << (4 * (gpio.pin - ((gpio.pin >= 8) * 8))));
}
