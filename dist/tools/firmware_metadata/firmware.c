#include <stdio.h>
#include <string.h>

int verify(int argc, char *argv[]);
extern const char verify_usage[];

int genkeys(int argc, char *argv[]);
extern const char genkeys_usage[];

int genmeta(int argc, char *argv[]);
extern const char genmeta_usage[];

int main(int argc, char *argv[])
{
    if (argc < 2) {
        goto usage;
    }
    if (!strcmp(argv[1], "verify")) {
        return verify(argc, argv);
    } else if (!strcmp(argv[1], "genkeys")) {
        return genkeys(argc, argv);
    } else if (!strcmp(argv[1], "genmeta")) {
        return genmeta(argc -1, &argv[1]);
    }

usage:
    fprintf(stderr, "usage: %s\n"  \
                    "       %s\n"  \
                    "       %s\n", \
                    verify_usage, genkeys_usage, genmeta_usage);
    return 1;
}
