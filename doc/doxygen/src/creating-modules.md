...
The main use cases for true-Pseudomodules is to provide base information for
...
generate a `.o` file grouped under that `MODULE`s `.a` file.
...
In `foo/Makefile` you add the source file conditioned on the Pseudomodule
inclusion
...
2. Using the `SUBMODULES` mechanism:
  ...
  # ensure that "foo_ham" or "bar_foo_fam" builds "foo_ham.c", not "foo_bar_ham.c"
  # or "bar_foo_bar_ham.c"
  BASE_MODULE := foo
...
been added, i.e. `USEMODULE += foo_ham foo_eggs` (its the same as case 1 but
