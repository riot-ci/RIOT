function initMenu(relPath,searchEnabled,serverSide,searchPage,search) {
    ...
    if (searchEnabled) {
        ...
        if (serverSide) {
            ...
            $('#riot-searchform div.form-group').append('<div id="MSearchBox" class="MSearchBoxInactive"><div class="input-group"><div class="input-group-addon"><span id="MSearchSelect" class="glyphicon glyphicon-search" aria-hidden="true" onmouseover="return searchBox.OnSearchSelectShow()" onmouseout="return searchBox.OnSearchSelectHide()"></span></div><input class="form-control" type="text" id="MSearchField" placeholder="'+search+'" value="'+search+'" accesskey="S" onfocus="searchBox.OnSearchFieldFocus(true)" onblur="searchBox.OnSearchFieldFocus(false)" onkeyup="searchBox.OnSearchFieldChange(event); var r=document.getElementById(\'MSearchResultsWindow\');if(parseInt(r.style.left)<0)r.style.left=0;var x=document.getElementById(\'MSearchResults\'); if (x.scrollWidth > window.screen.width) x.style.width=window.screen.width-2+\'px\';var f=document.getElementById(\'riot-searchform\');if(parseInt( r.style.top) < f.offsetTop+f.scrollHeight) r.style.top = f.offsetTop+f.scrollHeight+\'px\';"/><a id="MSearchClose" href="javascript:searchBox.CloseResultsWindow()"><span id="search-reset" class="glyphicon glyphicon-remove-circle" aria-hidden="true"></span></a></div></div>');
        }
    }
}
