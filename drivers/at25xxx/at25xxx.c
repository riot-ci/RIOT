...
int at25xxx_write(const at25xxx_t *dev, uint32_t pos, const void *data, size_t len)
{
    ...
    const uint32_t page_shift = 32 - __builtin_clz(PAGE_SIZE) - 1;
    ...
    while (len) {
        ssize_t written = at25xxx_write_page(dev, page, offset, d, len);
...
int at25xxx_set(const at25xxx_t *dev, uint32_t pos, uint8_t val, size_t len)
{
    ...
    while (len) {
        size_t written = at25xxx_write_page(dev, page, offset, data, min(len, sizeof(data)));
        len    -= written;
        page   += (offset + written) >> page_shift;
        offset  = (offset + written) & page_mask;
    }
