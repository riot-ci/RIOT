...
{
    assert(atwinc15x0);
    assert(!gpio_is_undef(atwinc15x0->params.reset_pin));
    assert(!gpio_is_undef(atwinc15x0->params.irq_pin));
    ...
    if (!gpio_is_undef(atwinc15x0->params.chip_en_pin)) {
        gpio_init(atwinc15x0->params.chip_en_pin, GPIO_OUT);
        gpio_set(atwinc15x0->params.chip_en_pin);
    }

    if (!gpio_is_undef(atwinc15x0->params.wake_pin)) {
