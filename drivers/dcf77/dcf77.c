 ...
 */
static uint8_t _read(bool *dest, gpio_t pin, bool startBit, bool endBit,
                     uint32_t threshold)
{
...
static int8_t _decodeInformation(uint8_t bitNumber, uint8_t result,
                                 dcf77_data_t *data)
{
    switch (bitNumber) {
        case  0:
        ...
        case HOUR_SINGLE:
            DEBUG("### HOUR SINGLE\n");
            data->hour = data->hour + (POTENZY1 * result);
            data->parity += result;
            break;
        case HOUR_SECOND:
            DEBUG("### HOUR_SECOND\n");
            data->hour = data->hour + (POTENZY2 * result);
            data->parity += result;
            break;
        case HOUR_QUAD:
            DEBUG("### HOUR QUAD\n");
            data->hour = data->hour + (POTENZY4 * result);
            data->parity += result;
            break;
        case HOUR_EIGHT:
            DEBUG("### HOUR EIGHT\n");
            data->hour = data->hour + (POTENZY8 * result);
            data->parity += result;
            break;
        case HOUR_TENNER:
            DEBUG("### HOUR TENNER\n");
            data->hour = data->hour + (POTENZY10 * result);
            data->parity += result;
            break;
        case HOUR_TEWENTIES:
            DEBUG("### HOUR TWENTIES\n");
            data->hour = data->hour + (POTENZY20 * result);
            data->parity += result;
            break;
        case HOUR_PR:
            DEBUG("### PARITY HOUR\n");
        ...
        case DAY_SINGLE:
            DEBUG("### calenderday SINGLE\n");
            data->calenderday = data->calenderday + (POTENZY1 * result);
            break;
        case DAY_SECOND:
            DEBUG("### DAY SECOND\n");
            data->calenderday = data->calenderday + (POTENZY2 * result);
            break;
        case DAY_QUAD:
            DEBUG("### DAY QUAD\n");
            data->calenderday = data->calenderday + (POTENZY4 * result);
            break;
        case DAY_EIGHT:
            DEBUG("### DAY EIGHT\n");
            data->calenderday = data->calenderday + (POTENZY8 * result);
            break;
        case DAY_TENNER:
            DEBUG("### DAY TENNER\n");
            data->calenderday = data->calenderday + (POTENZY10 * result);
            break;
        case DAY_TEWENTIES:
            DEBUG("### DAY TWENTIES\n");
            data->calenderday = data->calenderday + (POTENZY20 * result);
            DEBUG("###CALENDERDAY: %d\n", data->calenderday);
            break;
        case WEEKDAY_SINGLE:
 ...
 *
 * @param   dev               device
 * @param   data              datastruct for timeinformation
 *
 * @retval  DCF77_OK          Success
...
int dcf77_read(dcf77_t *dev, dcf77_data_t *data)
{
    ...
    for (int i = 0; i < READING_CYCLE; i++) {
        if (_read(&result, dev->params.pin, 1, 0,
                  PULSE_WIDTH_THRESHOLD) == DCF77_OK) {
            if (_decodeInformation(i, result, data) == DCF77_NOCSUM) {
                return DCF77_NOCSUM;
            }
        }
    }
