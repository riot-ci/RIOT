...
int hts221_init(hts221_t *dev, const hts221_params_t *params)
{
	...
	if (i2c_init_master(BUS, I2C_SPEED) < 0) {
		i2c_release(BUS);
		LOG_ERROR("hts221_init: i2c_init_master failed!\n");
		return -HTS221_NOBUS;
	}
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_WHO_AM_I, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_init: i2c_read_regs HTS221_REGS_WHO_AM_I failed!\n");
		return -HTS221_NOBUS;
	}
	if (reg != HTS221_DEVICE_ID) {
		i2c_release(BUS);
		LOG_ERROR("hts221_init: invalid HTS221_DEVICE_ID!\n");
		return -HTS221_NODEV;
	}
	i2c_release(BUS);
...
int hts221_one_shot(const hts221_t *dev)
{
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_CTRL_REG2, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_init: i2c_read_regs HTS221_REGS_CTRL_REG1 failed!\n");
		return -HTS221_NOBUS;
	}
...
int hts221_set_rate(const hts221_t *dev, const uint8_t rate)
{
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_CTRL_REG1, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_set_rate: i2c_read_regs HTS221_REGS_CTRL_REG1 failed!\n");
		return -HTS221_NOBUS;
	}
	reg |= rate;
	LOG_INFO("hts221_set_rate: %u\n", reg);
	if (i2c_write_regs(BUS, ADDR, HTS221_REGS_CTRL_REG1, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_set_rate: i2c_write_regs HTS221_REGS_CTRL_REG1 failed!\n");
		return -HTS221_NOBUS;
	}
	return HTS221_OK;
}
...
static int _set_power(const hts221_t *dev, const bool active)
{
	...
	if (i2c_write_regs(BUS, ADDR, HTS221_REGS_CTRL_REG1, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_init: i2c_write_regs HTS221_REGS_CTRL_REG1 failed!\n");
		return -HTS221_NOBUS;
	}
	return HTS221_OK;
}
...
int hts221_get_state(const hts221_t *dev)
{
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_STATUS_REG, &reg, 1) != 1) {
		i2c_release(BUS);
		LOG_ERROR("hts221_get_state: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	return (int)reg;
}
...
int hts221_read_humidity(const hts221_t *dev, uint16_t *val)
{
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_H0_RH_X2, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_humidity: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	h0_rh = reg[0] >> 1;
	h1_rh = reg[1] >> 1;
	/* 2. read h0_t0_out */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_H0_T0_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_humidity: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	...
	/* 3. read h1_t0_out */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_H1_T0_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_humidity: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	...
	/* 4. read humidity */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_HUMIDITY_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_humidity: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	h_t_out = (((uint16_t)reg[1]) << 8) | (uint16_t)reg[0];
	LOG_INFO("hts221_read_humidity: %i\n", h_t_out);
...
int hts221_read_temperature(const hts221_t *dev, int16_t *val)
{
	...
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_T0_DEGC_X8, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_temperature: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	/* 2. read t1_t0_msb */
	...
	/* 4. read t0_out */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_T0_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_temperature: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	...
	/* 5. read t1_out */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_T1_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_temperature: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	...
	/* 6. read t_out */
	if (i2c_read_regs(BUS, ADDR, HTS221_REGS_TEMP_OUT_L, &reg, 2) != 2) {
		i2c_release(BUS);
		LOG_ERROR("hts221_read_temperature: i2c_read_regs failed!\n");
		return -HTS221_NOBUS;
	}
	t_out = (((uint16_t)reg[1]) << 8) | (uint16_t)reg[0];
	LOG_INFO("hts221_read_temperature: %i\n", t_out);
