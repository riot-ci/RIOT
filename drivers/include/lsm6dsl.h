 ...
 */
typedef struct {
    uint8_t tap_ths;        /**< [4:0] threshold for tap detection (1 LSB = FS_XL/(2^5)) */
    uint8_t shock;          /**< [1:0] shock time window (1LSB = 8/ODR_XL) */
    uint8_t quiet;          /**< [1:0] quiet time (1LSB = 4/ODR_XL) */
    uint8_t dur;            /**< [3:0] max duration between two taps (1LSB = 32/ODR_XL) */
...
typedef struct {
    ...
    uint8_t wk_ths;     /**< [5:0] threshold for wake up (1 LSB = FS_XL/(2^6)) */
    uint8_t wake_dur;   /**< [1:0] Wake up duration event (1LSB = 1 ODR_time) */
    uint8_t sleep_dur;  /**< [3:0] sleep duration before event (1LSB = 512/ ODR_XL) */
 ...
 */
typedef struct {
    bool tap_ia;        /**< true when any type of TAP detected, false otherwise */
...
typedef struct {
    ...
    uint8_t acc_usr_ofs_w;                  /**< accelerometer user offset weigh */
 ...
 * @return < 0 on error
 */
int lsm6dsl_acc_get_wkup_src_alert(const lsm6dsl_t *dev, lsm6dsl_wkup_src_t *reason, bool disalble_flag);
