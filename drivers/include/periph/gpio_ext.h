 ...
 * However, it uses the functions of the newly defined low-level API, which
 * has be implemented by the MCU. The new GPIO API can therefore only be used
 * if the MCU implements the newly defined low-level API. This is automatically
...
static inline int gpio_init(gpio_t gpio, gpio_mode_t mode)
{
    ...
    return driver->init(gpio.port->port, gpio.pin, mode);
#else
    return gpio_cpu_init(gpio.port->port, gpio.pin, mode);
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    return driver->init_int(gpio.port->port, gpio.pin, mode, flank, cb, arg);
#else
    return gpio_cpu_init_int(gpio.port->port, gpio.pin, mode, flank, cb, arg);
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    driver->irq_enable(gpio.port->port, gpio.pin);
#else
    gpio_cpu_irq_enable(gpio.port->port, gpio.pin);
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    driver->irq_disable(gpio.port->port, gpio.pin);
#else
    gpio_cpu_irq_disable(gpio.port->port, gpio.pin);
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    return driver->read(gpio.port->port) & (1 << gpio.pin);
#else
    return gpio_cpu_read(gpio.port->port) & (1 << gpio.pin);
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    driver->set(gpio.port->port, (1 << gpio.pin));
#else
    gpio_cpu_set(gpio.port->port, (1 << gpio.pin));
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
   const gpio_driver_t *driver = gpio_driver_get(gpio);
   driver->clear(gpio.port->port, (1 << gpio.pin));
#else
   gpio_cpu_clear(gpio.port->port, (1 << gpio.pin));
#endif
}
...
#if MODULE_EXTEND_GPIO || DOXYGEN
    const gpio_driver_t *driver = gpio_driver_get(gpio);
    driver->toggle(gpio.port->port, (1 << gpio.pin));
#else
    gpio_cpu_toggle(gpio.port->port, (1 << gpio.pin));
#endif
}
