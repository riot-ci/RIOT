...
static uint8_t _compute_ldro(llcc68_t *dev)
{
    uint32_t symbol_len = (uint32_t)(1 << dev->mod_params.sf) / _bw_khz[dev->mod_params.bw - LLCC68_LORA_BW_125];
    if (symbol_len >= 16) {
        return 0x01;
    }
...
int llcc68_init(llcc68_t *dev)
{
    ...
    /* Default RX timeout is 0 (no timeout) */
    dev->rx_timeout = 0;
