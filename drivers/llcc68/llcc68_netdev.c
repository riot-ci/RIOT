...
static int _get(netdev_t *netdev, netopt_t opt, void *val, size_t max_len)
{
    ...
    switch(opt) {
        ...
        case NETOPT_DEVICE_TYPE:
            ...
            *((uint16_t *) val) = (pkt_type == LLCC68_PKT_TYPE_LORA) ? NETDEV_TYPE_LORA : NETDEV_TYPE_UNKNOWN;
...
static int _set_state(llcc68_t *dev, netopt_state_t state)
{
    switch (state) {
        case NETOPT_STATE_SLEEP:
            DEBUG("[llcc68] netdev: set NETOPT_STATE_SLEEP state\n");
            llcc68_set_sleep(dev, LLCC68_SLEEP_CFG_WARM_START);
            break;
