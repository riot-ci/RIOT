...
int lsm6dsl_acc_configure_wkup_alert(const lsm6dsl_t *dev, const lsm6dsl_wkup_params_t *params)
{
	assert(params);
    int res = 0;
    uint8_t tmp;
    ...
    tmp = (tmp & (~LSM6DSL_WAKE_UP_THS_WK_TH_MASK)) | (params->wk_ths & LSM6DSL_WAKE_UP_THS_WK_TH_MASK);
    res += i2c_write_reg(BUS, ADDR, LSM6DSL_REG_WAKE_UP_THS, tmp, 0);

    /* set free fall dur (4 bits over 5) and threshold */
    tmp = (params->ff_ths & LSM6DSL_FREE_FALL_FF_THS_MASK);
    tmp = ((params->ff_dur << LSM6DSL_FREE_FALL_FF_DUR_SHIFT)
            & LSM6DSL_FREE_FALL_FF_DUR_SHIFT);
...
int lsm6dsl_acc_enable_alert(lsm6dsl_t *dev, uint8_t alert, uint8_t pin, lsm6dsl_cb_t cb, void *arg)
{
    int res;
    uint8_t tmp;
    ...
    i2c_acquire(BUS);
    /* check pin */
    if (pin >= LSM6DSL_PIN_INT_MAX || dev->params.alert_pins[pin] == GPIO_UNDEF) {
        i2c_release(BUS);
        goto pin_error;
    }
    ...
    mdx_cfg_reg = (pin == LSM6DSL_PIN_INT_1 ? LSM6DSL_REG_MD1_CFG:LSM6DSL_REG_MD2_CFG);
    ...
    if (cb) {
        gpio_init_int(dev->params.alert_pins[pin], GPIO_IN, GPIO_RISING, cb, arg);
    }
    return LSM6DSL_OK;

pin_error:
    DEBUG("[ERROR] lsm6dsl_acc_enable_alert: selected pin (%d) not available\n", pin);
...
pin_error:
    DEBUG("[ERROR] lsm6dsl_acc_disable_alert: selected pin (%d) not available\n", pin);
    return -LSM6DSL_ERROR_CNF;
}

int lsm6dsl_acc_get_wkup_src_alert(const lsm6dsl_t *dev, lsm6dsl_wkup_src_t *reason, bool disable_flag)
{
    int res = 0;
    uint8_t tmp;
    /* read wake up source */
    ...
    if (reason) {
        ...
        reason->free_fall = (tmp & LSM6DSL_WAKE_UP_SRC_FF_IA_MASK) >> LSM6DSL_WAKE_UP_SRC_FF_IA_SHIFT;
        reason->inactive = (tmp & LSM6DSL_WAKE_UP_SRC_SLEEP_MASK) >> LSM6DSL_WAKE_UP_SRC_SLEEP_SHIFT;
        reason->active = (tmp & LSM6DSL_WAKE_UP_SRC_WU_IA_MASK) >> LSM6DSL_WAKE_UP_SRC_WU_IA_SHIFT;
        reason->x = (tmp & LSM6DSL_WAKE_UP_SRC_X_WU_MASK) >> LSM6DSL_WAKE_UP_SRC_X_WU_SHIFT;
        reason->y = (tmp & LSM6DSL_WAKE_UP_SRC_Y_WU_MASK) >> LSM6DSL_WAKE_UP_SRC_Y_WU_SHIFT;
...
int lsm6dsl_acc_configure_6D4D_alert(const lsm6dsl_t *dev, const lsm6dsl_6d4d_params_t *params)
{
    assert(params);
    int res = 0;
    uint8_t tmp;
...
int lsm6dsl_acc_get_6D4D_src_alert(const lsm6dsl_t *dev, lsm6dsl_6d4d_src_t *reason, bool disalble_flag)
{
    int res = 0;
    uint8_t tmp;
    i2c_acquire(BUS);
    if (reason) {
        ...
        reason->zh = (tmp & LSM6DSL_D6D_SRC_ZH_MASK) >> LSM6DSL_D6D_SRC_ZH_SHIFT;
        reason->zl = (tmp & LSM6DSL_D6D_SRC_ZL_MASK) >> LSM6DSL_D6D_SRC_ZL_SHIFT;
        reason->yh = (tmp & LSM6DSL_D6D_SRC_YH_MASK) >> LSM6DSL_D6D_SRC_YH_SHIFT;
        reason->yl = (tmp & LSM6DSL_D6D_SRC_YL_MASK) >> LSM6DSL_D6D_SRC_YL_SHIFT;
        reason->xh = (tmp & LSM6DSL_D6D_SRC_XH_MASK) >> LSM6DSL_D6D_SRC_XH_SHIFT;
...
int lsm6dsl_acc_configure_tap_alert(const lsm6dsl_t *dev, const lsm6dsl_tap_params_t *params)
{
    assert(params);
    int res = 0;
    uint8_t tmp;
    ...
    tmp |= ((params->dur << LSM6DSL_INT_DUR2_DUR_SHIFT)
...
int lsm6dsl_acc_get_tap_src_alert(const lsm6dsl_t *dev, lsm6dsl_tap_src_t *reason, bool disalble_flag)
{
    int res = 0;
    uint8_t tmp;
    i2c_acquire(BUS);
    if (reason) {
        res = i2c_read_reg(BUS, ADDR, LSM6DSL_REG_D6D_SRC, &tmp, 0);
        reason->tap_ia = (tmp & LSM6DSL_TAP_SRC_TAP_IA_MASK) >> LSM6DSL_TAP_SRC_TAP_IA_SHIFT;
        reason->single_tap = (tmp & LSM6DSL_TAP_SRC_S_TAP_MASK) >> LSM6DSL_TAP_SRC_S_TAP_SHIFT;
        reason->double_tap = (tmp & LSM6DSL_TAP_SRC_D_TAP_MASK) >> LSM6DSL_TAP_SRC_D_TAP_SHIFT;
        reason->tap_sign = (tmp & LSM6DSL_TAP_SRC_TAP_SIGN_MASK) >> LSM6DSL_TAP_SRC_TAP_SIGN_SHIFT;
        reason->x_tap = (tmp & LSM6DSL_TAP_SRC_X_TAP_MASK) >> LSM6DSL_TAP_SRC_X_TAP_SHIFT;
        reason->y_tap = (tmp & LSM6DSL_TAP_SRC_Y_TAP_MASK) >> LSM6DSL_TAP_SRC_Y_TAP_SHIFT;
