...
{
    int ret = mpl3115a2_read_pressure((mpl3115a2_t *)dev, &pressure, &state);
    if (ret != MPL3115A2_OK) {
        LOG_ERROR("[SAUL] mpl3115a2_read_pressure failed!\n");
        return -1;
    }
    /* pressure values are > 100000, hence /10 to fit into int16_t */
    res->val[0] = (int16_t)(pressure/10);
    res->unit = UNIT_PA;
    res->scale = 1;
...
{
    int ret = mpl3115a2_read_temp((mpl3115a2_t *)dev, &temperature);
    if (ret != MPL3115A2_OK) {
