...
static gnrc_pktsnip_t *_nrf24l01p_pkt_recv(gnrc_netif_t *netif)
{
    ...
    frame_len = netif->dev->driver->recv(netif->dev, frame_buffer,
                                             frame_len, NULL);
    if(frame_len <= 0) {
        DEBUG("[nrf24l01p] recv: driver error\n");
        return NULL;
    }
 ...
 */
static gnrc_pktsnip_t *_nrf24l01p_adpt_recv(gnrc_netif_t *netif)
{
    assert(netif);
    assert(netif->dev);
 ...
 */
static int _nrf24l01p_adpt_send(gnrc_netif_t *netif, gnrc_pktsnip_t *pkt)
{
    assert(netif);
    assert(pkt);
    assert(netif->dev);
    ...
    if (netif_hdr->flags & BCAST) {
        ...
        dst_addr = bcast_addr;
        dst_addr_len = sizeof(bcast_addr);
#if IS_USED(MODULE_NETSTATS_L2)
        ...
        dst_addr_len = netif_hdr->dst_l2addr_len;
    }
...
static const gnrc_netif_ops_t nrf24l01p_netif_ops = {
    ...
    .send = _nrf24l01p_adpt_send,
    .recv = _nrf24l01p_adpt_recv,
    .get = gnrc_netif_get_from_netdev,
    .set = gnrc_netif_set_from_netdev,
};

int gnrc_netif_nrf24l01p_create(gnrc_netif_t *netif, char *stack,
