 ...
 *
 * @file
 * @brief   Implementation of the public NRF24L01+ device interface
 *
...
int nrf24l01p_setup(nrf24l01p_t *dev, const nrf24l01p_params_t *params)
{
    ...
    if (dev->params.config.cfg_data_rate >= NRF24L01P_RF_DR_NUM_OF ||
        dev->params.config.cfg_crc == NRF24L01P_CRC_0BYTE ||
        dev->params.config.cfg_channel >= NRF24L01P_NUM_CHANNELS) {
        return -ENOTSUP;
    }
...
int nrf24l01p_set_enable_pipe(nrf24l01p_t *dev, nrf24l01p_pipe_t pipe,
                          bool enable)
{
    assert(dev);
    ...
    nrf24l01p_reg8_en_aa_t en_aa = (1 << pipe);
    nrf24l01p_reg8_dynpd_t dynpd = (1 << pipe);
    nrf24l01p_reg8_en_rxaddr_t en_rx_addr = (1 << pipe);
    nrf24l01p_acquire(dev);
    if (enable) {
        nrf24l01p_reg8_set(dev, NRF24L01P_REG_EN_AA, &en_aa);
        nrf24l01p_reg8_set(dev, NRF24L01P_REG_DYNPD, &dynpd);
        nrf24l01p_reg8_set(dev, NRF24L01P_REG_EN_RXADDR, &en_rx_addr);
    }
    else {
        nrf24l01p_reg8_clear(dev, NRF24L01P_REG_EN_AA, &en_aa);
        nrf24l01p_reg8_clear(dev, NRF24L01P_REG_DYNPD, &dynpd);
        nrf24l01p_reg8_clear(dev, NRF24L01P_REG_EN_RXADDR, &en_rx_addr);
    }
    nrf24l01p_release(dev);
    return 0;
}

int nrf24l01p_get_enable_pipe(nrf24l01p_t *dev, nrf24l01p_pipe_t pipe,
                              bool* enable)
{
    assert(dev);
    if (pipe >= NRF24L01P_PX_NUM_OF) {
    ...
    nrf24l01p_reg8_en_rxaddr_t en_rx_addr;
    nrf24l01p_acquire(dev);
    nrf24l01p_read_reg(dev, NRF24L01P_REG_EN_RXADDR, &en_rx_addr, 1);
    nrf24l01p_release(dev);
...
int nrf24l01p_set_channel(nrf24l01p_t *dev, uint8_t channel)
{
    ...
    nrf24l01p_reg8_rf_ch_t rf_ch = NRF24L01P_FLG_RF_CH(channel);
    nrf24l01p_acquire(dev);
    nrf24l01p_reg8_mod(dev, NRF24L01P_REG_RF_CH, NRF24L01P_MSK_RF_CH, &rf_ch);
    nrf24l01p_release(dev);
                             ...
                             nrf24l01p_pipe_t pipe)
{
    ...
    if (pipe == NRF24L01P_P0 || pipe == NRF24L01P_P1) {
        ...
        nrf24l01p_release(dev);
        memcpy(dev->params.urxaddr.arxaddr.rx_addr_long[pipe], addr,
               NRF24L01P_ADDR_WIDTH);
    }
    else {
        nrf24l01p_acquire(dev);
        nrf24l01p_write_reg(dev, reg_pipe_info[pipe].reg_pipe_addr, addr, 1);
        nrf24l01p_release(dev);
                             ...
                             nrf24l01p_pipe_t pipe)
{
    ...
    if (pipe == NRF24L01P_P0 || pipe == NRF24L01P_P1) {
        ...
        memcpy(addr, dev->params.urxaddr.arxaddr.rx_addr_long[NRF24L01P_P1],
               NRF24L01P_ADDR_WIDTH);
        addr[NRF24L01P_ADDR_WIDTH - 1]
