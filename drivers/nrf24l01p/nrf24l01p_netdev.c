...
static inline void _trigger_send(const nrf24l01p_t *dev)
{
...
static int _assert_awake(const nrf24l01p_t *dev)
{
...
netopt_state_t _state_to_netif(nrf24l01p_state_t state)
{
...
nrf24l01p_state_t _state_from_netif(netopt_state_t state)
{
...
static int _init(netdev_t *netdev)
{
    ...
    uint8_t addr_size = NRF24L01P_ADDR_WIDTH;
    uint8_t pipes = 0;
    uint8_t cnt = 1; /* counter assures that the LSB is different
                        for all pipe addresses*/
    uint8_t addr_p0[] = NRF24L01P_L2ADDR_AUTO;
    if (!memcmp(dev->params.urxaddr.rxaddrpx.rx_p0, addr_p0, addr_size)) {
        luid_base(dev->params.urxaddr.rxaddrpx.rx_p0, addr_size);
        dev->params.urxaddr.rxaddrpx.rx_p0[addr_size - 1] = cnt++;
    }
    memcpy(addr_p0, dev->params.urxaddr.rxaddrpx.rx_p0, addr_size);
    nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P0, addr_p0, addr_size);
    pipes |= (1 << NRF24L01P_P0);
    uint8_t addr_p1[] = NRF24L01P_L2ADDR_AUTO;
    if (!memcmp(dev->params.urxaddr.rxaddrpx.rx_p1, addr_p1, addr_size) ||
        addr_p0[addr_size - 1] ==
        dev->params.urxaddr.rxaddrpx.rx_p1[addr_size - 1]) {
        luid_base(dev->params.urxaddr.rxaddrpx.rx_p1, addr_size);
        dev->params.urxaddr.rxaddrpx.rx_p1[addr_size - 1] = cnt++;
    }
    memcpy(addr_p1, dev->params.urxaddr.rxaddrpx.rx_p1, addr_size);
    nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P1, addr_p1, addr_size);
    pipes |= (1 << NRF24L01P_P1);
    if (dev->params.urxaddr.rxaddrpx.rx_p2 != NRF24L01P_L2ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p2 = cnt++;
        nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P2,
                            &dev->params.urxaddr.rxaddrpx.rx_p2, 1);
        pipes |= (1 << NRF24L01P_P2);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p3 != NRF24L01P_L2ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p3 = cnt++;
        nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P3,
                            &dev->params.urxaddr.rxaddrpx.rx_p3, 1);
        pipes |= (1 << NRF24L01P_P3);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p4 != NRF24L01P_L2ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p4 = cnt++;
        nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P4,
                            &dev->params.urxaddr.rxaddrpx.rx_p4, 1);
        pipes |= (1 << NRF24L01P_P4);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p5 != NRF24L01P_L2ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p5 = cnt++;
        nrf24l01p_write_reg(dev, NRF24L01P_REG_RX_ADDR_P5,
                            &dev->params.urxaddr.rxaddrpx.rx_p5, 1);
    ...
    nrf24l01p_reg8_rf_setup_t rf_setup =
        NRF24L01P_FLG_RF_DR(dev->params.config.cfg_data_rate) |
    ...
    nrf24l01p_reg8_rf_ch_t rf_ch =
        NRF24L01P_FLG_RF_CH(dev->params.config.cfg_channel);
    nrf24l01p_reg8_config_t config =
        NRF24L01P_FLG_CRCO(dev->params.config.cfg_crc);
    nrf24l01p_reg8_features_t features = NRF24L01P_FLG_EN_DYN_ACK |
                                         NRF24L01P_FLG_EN_DPL     |
                                         NRF24L01P_FLG_EN_ACK_PAY;
...
/*
 ...
 +------------------------------------------------------+
 | destination address (NRF24L01P_ADDR_WIDTH Bytes)  ...
 +------------------------------------------------------+
 +------------------------------------------------------+
 ...
 */
    ...
    if (!buf && !len) {
        ...
        return NRF24L01P_ADDR_WIDTH + NRF24L01P_MAX_PAYLOAD_WIDTH;
    }
    nrf24l01p_t *dev = (nrf24l01p_t *)netdev;
    uint8_t pl_width;
...
/* In some rare cases the RX payload width (R_RX_PL_WID) exceeds
   ...
   and https://www.mikrocontroller.net/articles/NRF24L01_Tutorial */
    ...
    uint8_t dst_addr[NRF24L01P_ADDR_WIDTH];
    if (pno == NRF24L01P_P0) {
        memcpy(dst_addr, dev->params.urxaddr.rxaddrpx.rx_p0, sizeof(dst_addr));
    }
    else {
        memcpy(dst_addr, dev->params.urxaddr.rxaddrpx.rx_p1, sizeof(dst_addr));
        if (pno > NRF24L01P_P1) {
            dst_addr[NRF24L01P_ADDR_WIDTH - 1] =
...
static int _send(netdev_t *netdev, const iolist_t *iolist)
{
    ...
    nrf24l01p_write_reg(dev, NRF24L01P_REG_TX_ADDR, dst_addr, dst_addr_len);
    if (!memcmp(dst_addr, bcast_addr, dst_addr_len)) {
        ...
        nrf24l01p_reg8_setup_aw_t setup_aw =
            NRF24L01P_FLG_AW(nrf24l01p_valtoe_aw(dst_addr_len));
 ...
 */
static int _get(netdev_t *netdev, netopt_t opt, void *val,
                size_t max_len)
{
    nrf24l01p_t *dev = (nrf24l01p_t *)netdev;
    ...
    switch (opt) {
        ...
        case NETOPT_SRC_LEN: {
            ...
            *((uint16_t *)val) = NRF24L01P_ADDR_WIDTH;
            return sizeof(uint16_t);
        } break;
        case NETOPT_ADDRESS: {
            assert(max_len >= NRF24L01P_ADDR_WIDTH);
            memcpy(val, dev->params.urxaddr.rxaddrpx.rx_p0,
                   NRF24L01P_ADDR_WIDTH);
            return NRF24L01P_ADDR_WIDTH;
        } break;
        case NETOPT_AUTOACK: {
 ...
 * @retval -ENOTSUP             Unsupported netdev option @p opt
 */
static int _set(netdev_t *netdev, netopt_t opt, const void *val,
                size_t len)
{
    nrf24l01p_t *dev = (nrf24l01p_t *)netdev;
    ...
    switch (opt) {
        case NETOPT_ADDRESS: {
            ...
            assert(len == NRF24L01P_ADDR_WIDTH);
            int ret = nrf24l01p_set_rx_address(dev, val, NRF24L01P_P0);
        ...
        case NETOPT_STATE: {
            ...
            nrf24l01p_state_t s = _state_from_netif(*((netopt_state_t *)val));
