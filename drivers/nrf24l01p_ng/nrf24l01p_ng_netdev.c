...
int _init(netdev_t *netdev)
{
    ...
    if (spi_init_cs(dev->params.spi, dev->params.pin_cs) != SPI_OK) {
        DEBUG("[nrf24l01p_ng] _init(): spi_init_cs() failed\n");
        return -EIO;
    }
    if (gpio_init(dev->params.pin_ce, GPIO_OUT) < 0) {
        DEBUG("[nrf24l01p_ng] _init(): gpio_init() failed\n");
        return -EIO;
    }
    gpio_clear(dev->params.pin_ce);
    ...
    nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P0,
                           addr_p0, addr_size);
    pipes |= (1 << NRF24L01P_NG_P0);
    uint8_t addr_p1[] = NRF24L01P_NG_L2ADDR_AUTO;
    ...
    nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P1,
                           addr_p1, addr_size);
    pipes |= (1 << NRF24L01P_NG_P1);
    if (dev->params.urxaddr.rxaddrpx.rx_p2 != NRF24L01P_NG_ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p2 = cnt++;
        nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P2,
                               &dev->params.urxaddr.rxaddrpx.rx_p2, 1);
        pipes |= (1 << NRF24L01P_NG_P2);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p3 != NRF24L01P_NG_ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p3 = cnt++;
        nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P3,
                               &dev->params.urxaddr.rxaddrpx.rx_p3, 1);
        pipes |= (1 << NRF24L01P_NG_P3);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p4 != NRF24L01P_NG_ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p4 = cnt++;
        nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P4,
                            &dev->params.urxaddr.rxaddrpx.rx_p4, 1);
        pipes |= (1 << NRF24L01P_NG_P4);
    }
    if (dev->params.urxaddr.rxaddrpx.rx_p5 != NRF24L01P_NG_ADDR_UNDEF) {
        dev->params.urxaddr.rxaddrpx.rx_p5 = cnt++;
        nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_RX_ADDR_P5,
                            &dev->params.urxaddr.rxaddrpx.rx_p5, 1);
        pipes |= (1 << NRF24L01P_NG_P5);
    }
    uint8_t setup_aw =
    ...
    nrf24l01p_ng_write_reg(dev, NRF24L01P_NG_REG_EN_RXADDR, &pipes, 1);
    ...
    if (gpio_init_int(dev->params.pin_irq, GPIO_IN, GPIO_FALLING,
        ...
        DEBUG("[nrf24l01p_ng] _init(): gpio_init_int() failed\n");
        return -EIO;
    }
    return 0;
}
 ...
 */
    ...
    if (!pl_width ||
        ...
        DEBUG("[nrf24l01p_ng] RX error, flush RX FIFO\n");
/* In some rare cases the RX payload width (R_RX_PL_WID) exceeds
   ...
   and https://www.mikrocontroller.net/articles/NRF24L01_Tutorial */
    ...
    if (!buf) {
        DEBUG("[nrf24l01p_ng] Return exact frame length\n");
        return frame_len;
    }
    ...
    /* get received frame */
    DEBUG("[nrf24l01p_ng] Handle received frame\n");
...
int _send(netdev_t *netdev, const iolist_t *iolist)
{
    ...
    if (!(iolist->iol_base) || !(iolist->iol_next)) {
        DEBUG("[nrf24l01p_ng] No Tx address or no payload\n");
        return -ENOTSUP;
    }
    nrf24l01p_ng_t *dev = (nrf24l01p_ng_t *)netdev;
    uint8_t pl_width = 0;
    ...
    if (status & NRF24L01P_NG_FLG_IRQ) {
        DEBUG("[nrf24l01p_ng] Handle pending IRQ, before sending new data\n");
        nrf24l01p_ng_release(dev);
        _isr(&dev->netdev);
        return -EAGAIN;
    }
    ...
    for (const iolist_t *iol = iolist->iol_next; iol; iol = iol->iol_next) {
        if (pl_width + iol->iol_len > sizeof(payload)) {
            nrf24l01p_ng_release(dev);
            DEBUG("[nrf24l01p_ng] frame too big\n");
            return -E2BIG;
        }
...
void _isr(netdev_t *netdev)
{
    ...
    if (dev->state == NRF24L01P_NG_STATE_RX_MODE) {
        while (!(fifo_status & NRF24L01P_NG_FLG_RX_EMPTY)) {
            DEBUG("[nrf24l01p_ng] ISR: read pending Rx frames\n");
