...
#if !defined(PERIPH_UART_HAS_WRITE_BYTE)
...
#if !defined(PERIPH_UART_HAS_WRITE_STRING)
void uart_write_string(uart_t uart, const char *s)
{
    if (IS_ACTIVE(PERIPH_UART_HAS_WRITE_BYTE)) {
        while (*s) {
...
#if defined(PERIPH_UART_NEEDS_WRITE) && defined(PERIPH_UART_HAS_WRITE_BYTE)
