...
int ph_oem_init(ph_oem_t *dev, const ph_oem_params_t *params)
{
    ...
    /* Register read test */
    if (i2c_read_regs(I2C_DEV, ADDR, PH_OEM_REG_DEVICE_TYPE,
                      &reg_data, 1, 0x0) < 0) {
...
static int _unlock_address_reg(ph_oem_t *dev)
{
    ...
    i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_UNLOCK, 0x55, 0x0);
    i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_UNLOCK, 0xAA, 0x0);
    /* if successfully unlocked the register will equal 0x00 */
    i2c_read_reg(I2C_DEV, ADDR, PH_OEM_REG_UNLOCK, &reg_value, 0x0);
...
int ph_oem_set_i2c_address(ph_oem_t *dev, uint8_t addr)
{
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_ADDRESS, addr, 0x0) < 0) {
...
static int _set_interrupt_pin(const ph_oem_t *dev)
{
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_INTERRUPT, IRQ_OPTION, 0x0) < 0) {
...
int ph_oem_set_led_state(const ph_oem_t *dev, ph_oem_led_state_t state)
{
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_LED, state, 0x0) < 0) {
...
int ph_oem_set_device_state(const ph_oem_t *dev, ph_oem_device_state_t state)
{
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_HIBERNATE, state, 0x0) < 0) {
...
static int _new_reading_available(const ph_oem_t *dev)
{
    ...
    i2c_acquire(I2C_DEV);
    do {
        if (i2c_read_reg(I2C_DEV, ADDR, PH_OEM_REG_NEW_READING,
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_NEW_READING, 0x00, 0x0) < 0) {
...
int ph_oem_clear_calibration(const ph_oem_t *dev)
{
    ...
    assert(dev);
    i2c_acquire(I2C_DEV);
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_REQUEST, 0x01, 0) < 0) {
    ...
    do {
        if (i2c_read_reg(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_REQUEST, &reg_value,
                         0) < 0) {
            i2c_release(I2C_DEV);
            return PH_OEM_READ_ERR;
        }
                                  ...
                                  uint16_t calibration_value)
{
    ...
    if (i2c_write_regs(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0) < 0) {
    ...
    if (i2c_read_regs(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_BASE, &reg_value, 4, 0) < 0) {
                           ...
                           ph_oem_calibration_option_t option)
{
    ...
    if (i2c_write_reg(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_REQUEST,
                      option, 0) < 0) {
...
int ph_oem_read_calibration_state(const ph_oem_t *dev, uint16_t *calibration_state)
{
    ...
    if (i2c_read_reg(I2C_DEV, ADDR, PH_OEM_REG_CALIBRATION_CONFIRM,
                            ...
                            uint16_t temperature_compensation)
{
    ...
    if (i2c_write_regs(I2C_DEV, ADDR, PH_OEM_REG_TEMP_COMPENSATION_BASE,
                       &reg_value, 4, 0) < 0) {
                             ...
                             uint16_t *temperature_compensation)
{
    ...
    if (i2c_read_regs(I2C_DEV, ADDR, PH_OEM_REG_TEMP_CONFIRMATION_BASE,
                      &reg_value, 4, 0) < 0) {
...
int ph_oem_read_ph(const ph_oem_t *dev, uint16_t *ph_value)
{
    ...
    if (i2c_read_regs(I2C_DEV, ADDR, PH_OEM_REG_PH_READING_BASE,
                      &reg_value, 4, 0) < 0) {
