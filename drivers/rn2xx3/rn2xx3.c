...
int rn2xx3_mac_join_network(rn2xx3_t *dev, uint8_t mode)
{
    ...
    if (mode == LORAMAC_JOIN_ABP) {
        ...
        eeprom_addr |= addr[0] << 24;
        eeprom_addr |= addr[1] << 16;
        eeprom_addr |= addr[2] << 8;
        eeprom_addr |= addr[3];

        conf_addr |= dev->p.loramac.dev_addr[0] << 24;
        conf_addr |= dev->p.loramac.dev_addr[1] << 16;
        conf_addr |= dev->p.loramac.dev_addr[2] << 8;
        conf_addr |= dev->p.loramac.dev_addr[3];
