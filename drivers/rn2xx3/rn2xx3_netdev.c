 ...
 * @{
 * @file
 * @brief       Netdev adaptation for the rn2xx3 driver
 *
...
static int _init(netdev_t *netdev)
{
    ...
    if (rn2xx3_mac_join_network(dev, LORAMAC_DEFAULT_JOIN_PROCEDURE) != RN2XX3_REPLY_JOIN_ACCEPTED) {
        DEBUG("[rn2xx3] netdev: join procedure failed.\n");
        return -1;
    }
...
static int _get(netdev_t *netdev, netopt_t opt, void *val, size_t max_len)
{
    ...
    switch(opt) {
        ...
        case NETOPT_LINKCHECK_INTERVAL:
            assert(max_len >= sizeof(uint16_t));
            *((uint16_t*) val) = rn2xx3_mac_get_retx(dev);
            return sizeof(uint16_t);

        case NETOPT_RX1_WINDOW_DELAY:
            assert(max_len >= sizeof(uint16_t));
            *((uint16_t*) val) = rn2xx3_mac_get_rx1_delay(dev);
            return sizeof(uint16_t);

        case NETOPT_RX2_WINDOW_DELAY:
            assert(max_len <= sizeof(uint16_t));
            *((uint16_t*) val) = rn2xx3_mac_get_rx2_delay(dev);
            return sizeof(uint16_t);

        case NETOPT_AUTO_REPLY:
            assert(max_len >= sizeof(netopt_enable_t));
            *((netopt_enable_t*) val) = rn2xx3_mac_get_ar(dev) ? NETOPT_ENABLE : NETOPT_DISABLE;
            break;
...
static int _set(netdev_t *netdev, netopt_t opt, const void *val, size_t len)
{
    ...
    switch(opt) {
        ...
        case NETOPT_BATTERY_LEVEL:
            assert(len <= sizeof(uint8_t));
            rn2xx3_mac_set_battery(dev, *((const uint8_t*) val));
            return sizeof(uint8_t);

        case NETOPT_RETRANS:
            assert(len <= sizeof(uint8_t));
            rn2xx3_mac_set_retx(dev, *((const uint8_t*) val));
            return sizeof(uint8_t);

        case NETOPT_RX1_WINDOW_DELAY:
            assert(len <= sizeof(uint16_t));
            rn2xx3_mac_set_rx1_delay(dev, *((const uint16_t*) val));
            return sizeof(uint16_t);

        case NETOPT_AUTO_REPLY:
            assert(len <= sizeof(netopt_enable_t));
            rn2xx3_mac_set_ar(dev, *((const netopt_enable_t*) val) ? true : false);
            return sizeof(netopt_enable_t);
