...
static bool _tm_diff_overflow(struct tm *a, struct tm *b) {
    if (a->tm_yday - b->tm_yday > RTT_DAY_MAX) {
        return true;
    }

    if (a->tm_hour - b->tm_hour > RTT_HOUR_MAX) {
        return true;
    }

    if (a->tm_min - b->tm_min > RTT_MIN_MAX) {
        return true;
    }

    if (a->tm_sec - b->tm_sec > RTT_SEC_MAX) {
        return true;
    }
...
int rtc_set_alarm(struct tm *time, rtc_alarm_cb_t cb, void *arg)
{
    ...
    if (_tm_diff_overflow(time, &tm_now)) {
        return -1;
    }

    rtt_set_alarm((alarm - now) * RTT_SECOND, cb, arg);
