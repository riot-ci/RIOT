...
static uint16_t _do_measure(const si70xx_t *dev, uint8_t command)
{
    ...
    if (i2c_write_byte(SI70XX_I2C, SI70XX_ADDR, command, 0) != I2C_ACK) {
        DEBUG("[ERROR] Cannot write command '%d' to I2C.\n", command);
    }

    if (i2c_read_bytes(SI70XX_I2C, SI70XX_ADDR, result, 2, 0) != I2C_ACK) {
...
static uint64_t _get_serial(const si70xx_t *dev)
{
    ...
    if (i2c_write_bytes(SI70XX_I2C, SI70XX_ADDR, out, 2, 0) != I2C_ACK) {
        DEBUG("[ERROR] Cannot write command 'READ_ID_FIRST' to I2C.\n");
    }

    if (i2c_read_bytes(SI70XX_I2C, SI70XX_ADDR, in_first, 8, 0) != I2C_ACK) {
    ...
    if (i2c_read_bytes(SI70XX_I2C, SI70XX_ADDR, in_second, 8, 0) != I2C_ACK) {
...
static uint8_t _get_revision(const si70xx_t *dev)
{
    ...
    if (i2c_read_byte(SI70XX_I2C, SI70XX_ADDR, &in, 0) != I2C_ACK) {
...
int si70xx_init(si70xx_t *dev, const si70xx_params_t *params)
{
    ...
    if (i2c_write_byte(SI70XX_I2C, SI70XX_ADDR, SI70XX_RESET, 0) != I2C_ACK) {
