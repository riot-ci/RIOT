...
#define SRF08_DEV_I2V       (dev->params.i2c)
...
int srf08_set_max_range(const srf08_t *dev, uint8_t max_range)
{
    ...
    i2c_acquire(SRF08_DEV_I2V);
    status = i2c_write_reg(SRF08_DEV_I2V, SRF08_DEV_ADDR, SRF08_RANGE_REG, max_range, 0);
    /* Release the bus for other threads. */
    i2c_release(SRF08_DEV_I2V);
...
int srf08_set_max_gain(const srf08_t *dev, uint8_t gain)
{
    ...
    /* Acquire exclusive access to the bus. */
    i2c_acquire(SRF08_DEV_I2V);
    status = i2c_write_reg(SRF08_DEV_I2V, SRF08_DEV_ADDR, SRF08_GAIN_REG, gain, 0);
    /* Release the bus for other threads. */
    i2c_release(SRF08_DEV_I2V);
                        ...
                        int num_echos, srf08_mode_t ranging_mode)
{
    ...
    i2c_acquire(SRF08_DEV_I2V);
    /* set ranging mode */
    status = i2c_write_reg(SRF08_DEV_I2V, SRF08_DEV_ADDR, SRF08_COMMAND_REG,
                           ranging_mode, 0);
    /* Release the bus for other threads. */
    i2c_release(SRF08_DEV_I2V);
    ...
    for (register_location = 2; register_location < max_reg_no_read;
        ...
        i2c_acquire(SRF08_DEV_I2V);
        /* read the echo bytes */
        status = i2c_read_regs(SRF08_DEV_I2V, SRF08_DEV_ADDR, register_location,
