...
{
    DEBUG("[sx126x.c] sx126x_set_channel %" PRIu32 "Hz \n", freq);
    dev->channel = freq;
...
{
    DEBUG("[sx126x.c] sx126x_set_bandwidth %02x\n", bandwidth);
...
{
    DEBUG("[sx126x.c] sx126x_set_spreading_factor : %02x\n", sf);
...
{
    DEBUG("[sx126x.c] sx126x_set_coding_rate %01x\n", cr);
...
{
    DEBUG("[sx126x.c] sx126x_set_lora_payload_length %d\n", len);
...
{
    DEBUG("[sx126x.c] sx126x_set_lora_crc %d\n", crc);
...
{
    DEBUG("[sx126x.c] sx126x_set_lora_implicit_header %d\n", mode);
...
{
    DEBUG("[sx126x.c] sx126x_set_lora_preamble_length %" PRIu16 "\n", preamble);
...
{
    DEBUG("[sx126x.c] sx126x_set_lora_iq_invert %d\n", iq_invert);
