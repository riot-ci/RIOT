...
static int _range_cli_cmd(int argc, char **argv)
{
    ...
    if (!strcmp(argv[1], "start")) {
        ...
        dpl_callout_reset(&_rng_req_callout, RANGE_REQUEST_T);
 ...
 *
 * Processing should be kept to a minimum giving the interrupt context.
 ...
 *
 * @note The MAC extension interface is a link-list of callbacks,
 * subsequentcallbacks on the list will be not be called in the event
 * of returning true.
 *
 * @param[in] inst  Pointer to struct uwb_dev.
 * @param[in] cbs   Pointer to struct uwb_mac_interface.
 * *
 * @return true if valid recipient for the vent, false otherwise
 */
static bool _complete_cb(struct uwb_dev *inst, struct uwb_mac_interface *cbs)
{
    (void)cbs;
...
/**
 * @brief An event callback to send range request every RANGE_REQUEST_T.
