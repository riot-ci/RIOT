...
static void _join_abp(void)
{
    ...
    mibReq.Param.NetID = LORAMAC_DEFAULT_NETWORK_ID;
    LoRaMacMibSetRequestConfirm(&mibReq);
...
void semtech_loramac_set_network_id(uint32_t network_id)
{
    DEBUG("[semtech-loramac] set network ID %lu\n", network_id);
    MibRequestConfirm_t mibReq;
    mibReq.Type = MIB_NET_ID;
...
uint32_t semtech_loramac_get_network_id(void)
{
    DEBUG("[semtech-loramac] get network ID\n");
    MibRequestConfirm_t mibReq;
    mibReq.Type = MIB_NET_ID;
    LoRaMacMibGetRequestConfirm(&mibReq);
