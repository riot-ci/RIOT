                          ...
                          uint32_t auth_data_len, uint8_t X1[16])
{
	...
	uint8_t auth_data_len_in_encoded = (auth_data_len >= (uint32_t)block_size - len_encoding)?((uint32_t)block_size - len_encoding):auth_data_len;
        memcpy(auth_data_encoded + len_encoding, auth_data, auth_data_len_in_encoded);
	/* Calculate the MAC over the first block of AAD + heading length encoding */
        len = ccm_compute_cbc_mac(cipher, X1, auth_data_encoded, auth_data_len_in_encoded + len_encoding, X1);
        if (len < 0) {
            return -1;
        }
	...
	if(auth_data_len_in_encoded < auth_data_len) {
	  len = ccm_compute_cbc_mac(cipher, X1, auth_data + auth_data_len_in_encoded, auth_data_len - auth_data_len_in_encoded, X1);
	  if (len < 0) {
            return -1;
	  }
	}
