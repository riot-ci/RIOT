 ...
 */
#define ARCHITECTURE_WORD_BITS
#endif /* DOXYGEN */

#if (ARCHITECTURE_WORD_BITS == 8) || defined(DOXYGEN)
/**
 ...
 *
 * @details Synonym to `uint8_t`, `uint16_t` or `uint32_t` depending on architecture
 */
typedef uint8_t     uword_t;
/**
 * @brief   Word sized signed integer
 *
 * @details Synonym to `int8_t`, `int16_t` or `int32_t` depending on architecture
 */
