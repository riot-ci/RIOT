...
typedef enum {
    ...
    ENTROPY_SOURCE_ERR_TEST_BOTH    = -5,   /**< Repetition count and Adaptive proportion
                                             *   test error */
...
/**
 * @brief   Scale Min. Entropy to fixed point integer to avoid float. The entropy per sample
            (8 Byte) of a noise source can likely be smaller than 1 bit.
 */
#define ENTROPY_SOURCE_HMIN_SCALE(x)    ((unsigned)(x * (1 << 16)))

/**
 * @brief   Scale internal fixed point Min. Entropy back to float. This macro is not
 *          required and only there for convenience.
 */
...
/**
 * @brief   Abort factor for von Neumann extractor. The algorithms runs as long as no bit
 ...
 *
 * This function is typically used by the entropy source internally. A conditioning
 * component might need an interface to request a variable number of samples, e.g., depending
 * on the contained amount of entropy.
 *
 ...
 *
 * This function requests as many samples needed to create \p len unbiased bytes using
 * \p func. The algorithm compares bits of consecutive samples. Only bit changes will be
 * considered for the output value. An abort criterium stops sampling after
 * (len * CONFIG_ENTROPY_SOURCE_NEUMANN_ABORT) iterations.
 *
 * @warning This function has a non-deterministic runtime.
 *
 * @param[in] func   pointer to @ref entropy_source_sample_func_t function that returns samples
 ...
 * <a href="https://github.com/usnistgov/SP800-90B_EntropyAssessment">SP800-90B EntropyAssessment</a>.
 * a: Probability of type I error. We assume 2^(-20).
 *
 * @param[in] entropy_per_sample   Estimated min. entropy of one sample scaled by
 *                                 ENTROPY_SOURCE_HMIN_SCALE()
 *
 * @return                         Cutoff threshold
 */
static inline int entropy_source_test_rep_cutoff(unsigned entropy_per_sample)
{
    return (1 + ((20 * 65536) / entropy_per_sample));
}

/**
 * @brief   Calculate cutoff value for Adaptive Proportion Test (NIST SP 800-90B 4.4.2)
 *
 * @param[in] entropy_per_sample   Estimated min. entropy of one sample scaled by
 *                                 ENTROPY_SOURCE_HMIN_SCALE()
 *
 * @return                         Cutoff value
 * @return                         ENTROPY_SOURCE_ERR_CONFIG if parameter invalid
 */
static inline int entropy_source_test_prop_cutoff(unsigned entropy_per_sample)
{
    int ret;

    if (entropy_per_sample < 65536) {        /* 1 bit/sample */
        ret = 410;
    }
    else if (entropy_per_sample < 131072) {  /* 2 bit/sample */
        ret = 311;
    }
    else if (entropy_per_sample < 131072) {  /* 4 bit/sample */
        ret = 177;
    }
    else if (entropy_per_sample < 131072) {  /* 8 bit/sample */
        ret = 62;
    }
    else if (entropy_per_sample == 131072) { /* 8 bit/sample */
        ret = 13;
 ...
 * @param[in]      c_rep   Cutoff value calculated by @ref entropy_source_test_rep_cutoff.
 */
 ...
 *
 * @param[in, out] state   Test structure of one entropy source.
 * @param[in]      c_prop  Cutoff value calculated by @ref entropy_source_test_prop_cutoff.
 */
 ...
 * This function will not block the sampling. It only indicates detected errors.
 *
 * @param[in, out] state_rep    Repetition Count test structure of one entropy source.
 * @param[in, out] state_prop   Adaptive Proportion test structure of one entropy source.
 * @param[in]      sample       Current sample.
 *
 * @return          ENTROPY_SOURCE_OK on success
 * @return          negative @ref entropy_source_error_t code on error
