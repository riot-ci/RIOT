 ...
 */
static inline bool gnrc_netdev_get_rx_started(gnrc_netif2_t *netif)
{
    return (netif->mac.mac_info & GNRC_NETDEV_MAC_INFO_RX_STARTED);
}
 ...
 *
 */
static inline void gnrc_netdev_set_rx_started(gnrc_netif2_t *netif, bool rx_started)
{
    if (rx_started) {
        netif->mac.mac_info |= GNRC_NETDEV_MAC_INFO_RX_STARTED;
    }
    else {
        netif->mac.mac_info &= ~GNRC_NETDEV_MAC_INFO_RX_STARTED;
    }
 ...
 */
static inline gnrc_mac_tx_feedback_t gnrc_netdev_get_tx_feedback(gnrc_netif2_t *netif)
{
 ...
 *
 */
static inline void gnrc_netdev_set_tx_feedback(gnrc_netif2_t *netif,
