...
#define PAGE_FILTER(page_str) {                                 \
            ...
            .value = page_str, .value_len = sizeof(page_str)    \
        }
...
/* append default filters to requests, do raw lookup and decodes the raw response */
...
static ssize_t _add_filters_to_lookup(coap_pkt_t *pkt, cord_lc_filter_t *filters)
{
    ...
    while (f) {
        for (unsigned i = 0; i < f->len; i++) {
            ...
            if (gcoap_add_qstring2(pkt, kv->key, kv->key_len, kv->value, kv->value_len) == -1) {
                return CORD_LC_OVERFLOW;
            }
        }
        f = f->next;
