...
static uint8_t _rxbuf[GCOAP_PDU_BUF_SIZE];
...
static void _listen(sock_udp_t *sock)
{
     ...
     * _event_loop(). */
    ssize_t res = sock_udp_recv(sock, _rxbuf, sizeof(_rxbuf),
    ...
    case COAP_CLASS_REQ:
        if (coap_get_type(&pdu) == COAP_TYPE_NON
            ...
            size_t pdu_len = _handle_req(&pdu, _rxbuf, sizeof(_rxbuf), &remote);
            if (pdu_len > 0) {
                ssize_t bytes = sock_udp_send(sock, _rxbuf, pdu_len, &remote);
                if (bytes <= 0) {
