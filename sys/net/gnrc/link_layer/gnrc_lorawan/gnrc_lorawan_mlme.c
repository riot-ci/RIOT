...
void gnrc_lorawan_mlme_process_join(gnrc_lorawan_t *mac, gnrc_pktsnip_t *pkt)
{
    ...
    mac->mlme.nid = ja_hdr->net_id[0] | ja_hdr->net_id[1] << 8 | ja_hdr->net_id[2] << 16;
    /* Copy devaddr */
...
uint8_t gnrc_lorawan_build_options(gnrc_lorawan_t *mac, lorawan_buffer_t *buf)
{
    ...
    size += mac->mlme.pending_mlme_opts & GNRC_LORAWAN_MLME_OPTS_LINK_CHECK_REQ ? _fopts_mlme_link_check_req(mac, buf) : 0;
    return size;
}
