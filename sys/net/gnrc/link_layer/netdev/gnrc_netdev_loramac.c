 ...
 *
 * @file
 * @brief       GNRC to netdev adapter for LoRaMAC devices
 *
...
#include "assert.h"
#include "net/gnrc.h"
#include "net/gnrc/netdev/loramac.h"
...
static gnrc_pktsnip_t _recv(gnrc_netdev_t *gnrc_netdev)
{
    ...
    if (pktlen <= 0) {
        DEBUG("[loramac-gnrc] recv: no data available to process\n");
        return NULL;
    }
    ...
    if (payload == NULL) {
        DEBUG("[loramac-gnrc] recv: unable to allocate space in the pktbuf\n");
...
static int _send(gnrc_netdev_t *dev, gnrc_pktsnip_t *pkt)
{
    ...
    DEBUG("[loramac-gnrc] send: payload of packet is %i\n", (int)size);
    ...
    if (vec != NULL) {
        ...
        DEBUG("[loramac-gnrc] send: triggering the mac send function\n");
        res = dev->dev->driver->send(dev->dev, vector, count);
        if (res == -ENOTSUP) {
            DEBUG("[loramac-gnrc] send: cannot send, mac is busy");
        }
    }
    else {
        DEBUG("[loramac-gnrc] send: unable to create iovec\n");
    }
...
void gnrc_netdev_loramac_init(gnrc_netdev_t *gnrc_netdev, netdev_t *dev)
{
