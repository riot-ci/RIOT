...
static size_t _fit(const gnrc_pktsnip_t *orig_pkt)
{
    ...
    gnrc_pktsnip_t *netif_hdr = gnrc_pktsnip_search_type((gnrc_pktsnip_t *)orig_pkt,
                                                         GNRC_NETTYPE_NETIF);
    /* dito */
    size_t pkt_len = gnrc_pkt_len((gnrc_pktsnip_t *)orig_pkt) +
                     ICMPV6_ERROR_SZ + sizeof(ipv6_hdr_t);
                                           ...
                                           uint32_t value)
{
    ...
    bool send_order = _check_send_order(orig_pkt);
