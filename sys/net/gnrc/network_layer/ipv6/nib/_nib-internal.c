...
#if GNRC_IPV6_NIB_CONF_MULTIHOP_P6C
    unsigned idx = INDEX_OF(_dsts, nib_offl);
...
void _nib_abr_add_pfx(_nib_abr_entry_t *abr, const _nib_offl_entry_t *offl)
{
    ...
    unsigned idx = INDEX_OF(_dsts, offl);
...
{
    if ((last == NULL) ||
        (INDEX_OF(_dsts, last) < GNRC_IPV6_NIB_OFFL_NUMOF)) {
        ...
        while ((ptr = _nib_offl_iter(ptr))) {
            ...
            if ((ptr->mode & _PL) && (bf_isset((uint8_t *)abr->pfxs, INDEX_OF(_dsts, ptr)))) {
                return ptr;
            }
        }
    }
