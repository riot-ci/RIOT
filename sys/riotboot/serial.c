...
/* we send two characters each iteration -> 16 bit + start & stop bit */
#define RIOTBOOT_DELAY (RIOTBOOT_DELAY_MS * RIOTBOOT_UART_BAUDRATE / 18000)
...
static bool _bootdelay(unsigned tries, volatile bool *boot_default)
{
    ...
    while (--tries && *boot_default) {
        uart_write(RIOTBOOT_UART_DEV, (uint8_t*)"b\b", 2);
    }
