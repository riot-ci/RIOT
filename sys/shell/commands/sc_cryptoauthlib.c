...
int _cryptoauth(int argc, char **argv)
{
    if (argc > 1) {
        ...
        printf("* read - read atca device's config zone\n");
        printf("* lock_c - lock atca device's config zone\n");
        printf("* check_lc - check if atca device's config zone is locked\n");
        printf("* check_ld - check if atca device's data zone is locked\n");
    }
    return 0;
}
