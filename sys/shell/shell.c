...
static void handle_input_line(const shell_command_t *command_list, char *line)
{
    ...
    *writepos = '\0';
    ...
    if (handler != NULL) {
        if (IS_USED(MODULE_SHELL_HOOKS)) {
            pre_command_hook(argc, argv);
            int res = handler(argc, argv);
            post_command_hook(res, argc, argv);
        }
        else {
            handler(argc, argv);
        }
...
__attribute__((weak)) void post_readline_hook(void)
{

}

__attribute__((weak)) void pre_command_hook(int argc, char **argv)
{
    (void)argv;
    (void)argc;
}

__attribute__((weak)) void post_command_hook(int ret, int argc, char **argv)
{
    (void)ret;
    (void)argv;
    (void)argc;
