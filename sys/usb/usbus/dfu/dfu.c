...
void usbus_dfu_init(usbus_t *usbus, usbus_dfu_device_t *handler, unsigned mode)
{
    ...
    handler->dfu_state = (handler->mode == USB_DFU_PROTOCOL_DFU_MODE) ?
                USBUS_DFU_STATE_DFU_IDLE : USBUS_DFU_STATE_APP_IDLE;
    ...
    switch(pkt->request) {
        ...
        case DFU_DOWNLOAD:
            ...
            if (pkt->length == 0) {
                ...
                dfu->dfu_state = USBUS_DFU_STATE_DFU_MANIFEST_SYNC;
                riotboot_flashwrite_flush(&dfu->writer);
                riotboot_flashwrite_finish(&dfu->writer);
            }
            else if (dfu->dfu_state != USBUS_DFU_STATE_DFU_DL_SYNC) {
                dfu->dfu_state = USBUS_DFU_STATE_DFU_DL_SYNC;
            }
            else {
        ...
        case DFU_GET_STATUS:
        {
            ...
            if (dfu->dfu_state == USBUS_DFU_STATE_DFU_DL_SYNC) {
                dfu->dfu_state = USBUS_DFU_STATE_DFU_DL_IDLE;
                DEBUG("GET STATUS GO TO IDLE\n");
            }
            else if (dfu->dfu_state == USBUS_DFU_STATE_DFU_MANIFEST_SYNC) {
