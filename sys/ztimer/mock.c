...
{
    DEBUG("zmock_advance: now=0x%08" PRIx32 " + 0x%08" PRIx32 ", target=0x%08" PRIx32 " (%u)\n",
        self->now, val, self->target, self->armed);
    self->now = (self->now + val) & self->mask;
    if (self->armed) {
        /* Update target */
        if (val >= self->target) {
            /* Target was hit */
...
{
    *self = (ztimer_mock_t){
        .mask = (~((uint32_t)0ul)) >> (32 - width),
        .super = { .ops = &ztimer_mock_ops, },
    };
