...
def testfunc(child):
    ...
    for k in thread_prio.keys():
        child.expect(r"T%i \(prio %i\): waiting on condition variable now" % (k, thread_prio[k]))

    count = 0
    last = -1
    child.expect(r"First batch was signaled")
    for _ in range(len(thread_prio)):
        ...
        count += 1
        if count == 3:
            child.expect(r"First batch has woken up")
            child.expect(r"Second batch was signaled")
    child.expect(r"Second batch has woken up")
