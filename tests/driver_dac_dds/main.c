...
static void play_function(uint16_t period, uint32_t samples, sample_gen_t fun)
{
    ...
    dac_dds_set_cb(DAC_CHAN, _unlock, &lock);

    while (samples) {
        size_t len = min(samples, len_aligned);
        samples -= len;

        dac_dds_play(DAC_CHAN, buf, len);
...
static int cmd_greeting(int argc, char **argv)
{
    ...
    dac_dds_set_cb(DAC_CHAN, NULL, NULL);

    dac_dds_play(DAC_CHAN, hello_raw, hello_raw_len);
...
static void _dac_init(void)
{
    ...
    dac_dds_init(DAC_CHAN, sample_rate,
