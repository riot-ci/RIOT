...
static void lis2dh12_int_cb(void* l) {
    ...
    DEBUG("[INT]: INT_SRC - IA %d; ZH %d; ZL %d; YH %d; YL %d; XH %d; XL %d.\n",
            int1_src & LIS2DH12_INT_SRC_IA, int1_src & LIS2DH12_INT_SRC_ZH, int1_src & LIS2DH12_INT_SRC_ZL, int1_src & LIS2DH12_INT_SRC_YH,
            int1_src & LIS2DH12_INT_SRC_YL, int1_src & LIS2DH12_INT_SRC_XH, int1_src & LIS2DH12_INT_SRC_XL);
...
void lis2dh12_test_init(void) {
    ...
    if (gpio_is_valid(pin1)) {
        params_int1.int_config = LIS2DH12_INT_CFG_XHIE | LIS2DH12_INT_CFG_YHIE | LIS2DH12_INT_CFG_ZHIE;
...
void* lis2dh12_test_process(void* arg) {
    ...
    while (1){
        ...
        /* after Interrupt, device needs to enable FIFO again, (sets to BYPASS and back to previous FIFO mode) */
...
int shell_lis2dh12_cmd(int argc, char **argv) {
    ...
    printf("Command: lis %s %s\n", (argc > 1) ? argv[1] : "",
           (argc > 2) ? argv[2] : "");
    const char *const usage = "USAGE: lis <subcommand> [arg], with subcommand "
    ...
    /* set commands */
        ...
        if (line == 1) {
            if (!thold) {
                ...
                params_int1.int_config = LIS2DH12_INT_CFG_XHIE | LIS2DH12_INT_CFG_YHIE
