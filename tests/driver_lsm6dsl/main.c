...
static void _usage_configure(void)
{
    ...
    puts("              uint8_t wake_dur;   /**< [1:0] Wake up duration event (1LSB = 1 ODR_time) */");
    puts("              uint8_t sleep_dur;  /**< [3:0] sleep duration before event (1LSB = 512/ ODR_XL) */");
    puts("");
    ...
    puts("              uint8_t tap_ths;        /**< [4:0] threshold for tap detection (1 LSB = FS_XL/(2^5)) */");
    puts("              uint8_t shock;          /**< [1:0] shock time window (1LSB = 8/ODR_XL) */");
...
static int _lsm6dsl_configure_handler(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "wkup_int") == 0 || strcmp(argv[1], "sleep_int") == 0) {
...
{
    printf("Usage: enable <wkup_int | sleep_int | 6d4d_int | ff_int | tap1_int | tap2_int> <pin> \n");
    puts("");
}
...
static int _lsm6dsl_enable_handler(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "wkup_int") == 0) {
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_WAKE_UP, atoi(argv[2]), alert_cb, "WakeUp") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_SLEEP, atoi(argv[2]), alert_cb, "Sleep") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        puts(" To trig the alert, move the device and stop, wait configured sleep duration");
    }
    else if (strcmp(argv[1], "ff_int") == 0) {
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_FREE_FALL, atoi(argv[2]), alert_cb, "FreeFall") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        puts(" To trig the alert, release your device at the right height to ger a free-fall...");
    }
    else if (strcmp(argv[1], "6d4d_int") == 0) {
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_6D_4D, atoi(argv[2]), alert_cb, "6D4D") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_SINGLE_TAP, atoi(argv[2]), alert_cb, "SingleTAP") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_enable_alert(&dev, LSM6DSL_INT_SINGLE_TAP, atoi(argv[2]), alert_cb, "DoubleTAP") != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
...
{
    printf("Usage: disable <wkup_int | sleep_int | 6d4d_int | ff_int | tap1_int | tap2_int> <pin> \n");
    puts("");
}
...
static int _lsm6dsl_disable_handler(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "wkup_int") == 0) {
        if (lsm6dsl_acc_disable_alert(&dev, LSM6DSL_INT_WAKE_UP, atoi(argv[2])) != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_disable_alert(&dev, LSM6DSL_INT_SLEEP, atoi(argv[2])) != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_disable_alert(&dev, LSM6DSL_INT_FREE_FALL, atoi(argv[2])) != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_disable_alert(&dev, LSM6DSL_INT_6D_4D, atoi(argv[2])) != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
        ...
        if (lsm6dsl_acc_disable_alert(&dev, LSM6DSL_INT_SINGLE_TAP, atoi(argv[2])) != LSM6DSL_OK) {
            puts("[ERROR]");
            return 1;
        }
        puts("[SUCCESS]\n");
