    ...
    switch (argc) {
        case 6: {
            ...
            if ((!strcmp(argv[2], "-s")) || (!strcmp(argv[2], "--set"))) {
                if (!(strcmp(argv[3], "ack_payload"))) {
                    ...
                    uint8_t address[NRF24L01P_MAX_ADDR_WIDTH];
                    char *tok = NULL;
                    int a = 0;
                    printf("%s\n", argv[4]);
        ...
        case 5: {
            ...
            /* nrf24ctl <dev_index> --get <attribute> <pipe> */
                ...
                if (!(strcmp(argv[3], "address"))) {
                    uint8_t address[2 * NRF24L01P_MAX_ADDR_WIDTH + 5];
                    address[sizeof(address) - 1] = '\0';
                    ret = nrf24l01p_get_rx_address(dev, address, pipe);
                    uint8_t i;
                    ...
                    uint8_t plw = nrf24l01p_get_mtu(dev, pipe);
                    printf("%u\n", plw);
                }
                else {
        ...
        case 4: {
            ...
            if ((!strcmp(argv[2], "-g")) || (!strcmp(argv[2], "--get"))) {
                if (!(strcmp(argv[3], "channel"))) {
                    uint8_t ch = nrf24l01p_get_channel(dev);
                    printf("nrf24l01p device %d - Channel: %u (%d GHz)\n",
                           dev_index, ch, NRF24L01P_BASE_FRQ_MHZ + ch);
                }
                else if (!(strcmp(argv[3], "crc"))) {
                    uint8_t crc_len = nrf24l01p_get_crc(dev, NULL);
                    printf("nrf24l01p device %d - CRC length: %u [bytes]\n",
                           dev_index,  crc_len);
                }
                else if (!(strcmp(argv[3], "data_rate"))) {
                    ...
                    int8_t power = nrf24l01p_get_tx_power(dev, NULL);
                    printf("nrf24l01p device %d - Tx Power: %d [dbm]\n",
                           dev_index, power);
                }
                else if (!(strcmp(argv[3], "retr_delay"))) {
                    ...
                    const char *sstate = nrf24l01p_state_to_string(dev->state);
                    printf("nrf24l01p device %d - State: %s\n", dev_index,
                           sstate);
                }
                else if (!(strcmp(argv[3], "max_retr"))) {
                    uint8_t retransm = nrf24l01p_get_max_retransm(dev);
                    printf("nrf24l01p device %d - Maximum retransmission: %u\n",
                           dev_index, retransm);
                }
                else {
                    printf("[nrf24ctl] Invalid attribute: --get %s\n",
