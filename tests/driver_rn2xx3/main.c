...
inline void _print_sys_usage(void)
{
    puts("Usage: mac <sleep|reset|factoryRESET>");
}

inline void _print_mac_usage(void)
{
    puts("Usage: mac <join|tx|set|get|save>");
}

inline void _print_mac_get_usage(void)
{
    puts("Usage: mac get "
...
inline void _print_mac_set_usage(void)
{
    puts("Usage: mac set "
...
int rn2xx3_mac_cmd(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "join") == 0) {
        ...
        if (strcmp(argv[2], "deveui") == 0) {
            rn2xx3_mac_get_dev_eui(&rn2xx3_dev, payload);
            memset(rn2xx3_dev.resp_buf, 0, strlen(rn2xx3_dev.resp_buf));
            rn2xx3_bytes_to_hex(payload, rn2xx3_dev.resp_buf, LORAMAC_DEVEUI_LEN);
            printf("device eui: %s\n", rn2xx3_dev.resp_buf);
        }
        else if (strcmp(argv[2], "appeui") == 0) {
            rn2xx3_mac_get_app_eui(&rn2xx3_dev, payload);
            memset(rn2xx3_dev.resp_buf, 0, strlen(rn2xx3_dev.resp_buf));
            rn2xx3_bytes_to_hex(payload, rn2xx3_dev.resp_buf, LORAMAC_APPEUI_LEN);
            printf("application eui: %s\n", rn2xx3_dev.resp_buf);
        }
        else if (strcmp(argv[2], "devaddr") == 0) {
            rn2xx3_mac_get_dev_addr(&rn2xx3_dev, payload);
            memset(rn2xx3_dev.resp_buf, 0, strlen(rn2xx3_dev.resp_buf));
            rn2xx3_bytes_to_hex(payload, rn2xx3_dev.resp_buf, LORAMAC_DEVADDR_LEN);
            ...
            uint8_t mode = rn2xx3_mac_get_tx_mode(&rn2xx3_dev);
            printf("TX mode: ");
            switch (mode) {
                case LORAMAC_TX_CNF:
                    printf("confirmable");
            ...
            uint8_t idx = rn2xx3_mac_get_tx_power(&rn2xx3_dev);
            printf("pwridx: %d\n", idx);
        }
        else if (strcmp(argv[2], "dr") == 0) {
            uint8_t dr = rn2xx3_mac_get_dr(&rn2xx3_dev);
            printf("dr: %d\n", dr);
        }
            ...
            uint8_t rx2_dr = rn2xx3_mac_get_rx2_dr(&rn2xx3_dev);
            printf("rx2 dr: %d\n", rx2_dr);
        }
        ...
        if (strcmp(argv[2], "deveui") == 0) {
            rn2xx3_hex_to_bytes(argv[3], payload);
            rn2xx3_mac_set_dev_eui(&rn2xx3_dev, payload);
        }
        else if (strcmp(argv[2], "appeui") == 0) {
            rn2xx3_hex_to_bytes(argv[3], payload);
            rn2xx3_mac_set_app_eui(&rn2xx3_dev, payload);
        }
        else if (strcmp(argv[2], "appkey") == 0) {
            rn2xx3_hex_to_bytes(argv[3], payload);
            rn2xx3_mac_set_app_key(&rn2xx3_dev, payload);
        }
        else if (strcmp(argv[2], "devaddr") == 0) {
            rn2xx3_hex_to_bytes(argv[3], payload);
            rn2xx3_mac_set_dev_addr(&rn2xx3_dev, payload);
        }
        else if (strcmp(argv[2], "txport") == 0) {
