...
int main(void)
{
    ...
    sdp3x_start_continuous(&sdp3x_dev, SDP3X_FLAG_DIFF_PRESS|SDP3X_FLAG_AVERAGE);
    ...
    while (i < (uint32_t)TEST_ITERATIONS) {
        printf(
            ...
            (double)result.temperature/100.0, (double)result.differential_pressure/100.0);
        xtimer_sleep(1);
        i++;
    }
     ...
     */
    while (i < (uint32_t)TEST_ITERATIONS) {
        sdp3x_read_single_measurement(&sdp3x_dev, SDP3X_FLAG_DIFF_PRESS, &result);
        printf(
