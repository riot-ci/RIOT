...
static event_periodic_timeout_t event_periodic;
...
static void callback_4times(void *arg)
{
    ...
    *count = *count + 1;
    ...
    if (*count == 4) {
        event_periodic_timeout_stop(&event_periodic);
        mutex_unlock(&lock);
    }
    else if (*count > 4) {
...
int main(void)
{
    ...
    event_periodic_timeout_init(&event_periodic, ZTIMER_USEC,
                                EVENT_PRIO_MEDIUM, &event_4times.super);
    before = ztimer_now(ZTIMER_USEC);
    event_periodic_timeout_start(&event_periodic, EVENT_TIMEOUT_TIME);
