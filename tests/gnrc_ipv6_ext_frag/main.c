...
static void test_ipv6_ext_frag_fill_last_fragment(const ipv6_addr_t *dst)
{
    gnrc_pktsnip_t *pkt;
    unsigned udp_mtu = netif->ipv6.mtu - sizeof(ipv6_ext_frag_t) -
                       sizeof(ipv6_hdr_t);
    /* 1st part: lower to multiple of 8 for 1st fragment */
    unsigned payload_size = ((udp_mtu - sizeof(udp_hdr_t)) & 0xfff8) +
                            udp_mtu;          /* 2nd fragment to the max */

    TEST_ASSERT_NOT_NULL(local_addr);
    pkt = _build_udp_packet(dst, payload_size, NULL);
    TEST_ASSERT_NOT_NULL(pkt);
    gnrc_ipv6_ext_frag_send_pkt(pkt, netif->ipv6.mtu);
}
...
static int shell_test_cmd(int argc, char **argv)
{
    ...
    static void (* const _shell_tests[])(const ipv6_addr_t *) = {
        ...
        test_ipv6_ext_frag_fill_last_fragment,
    };
    int test_num;
    ...
    if ((unsigned)test_num >=
        ...
        printf("<num> must be between 0 and %u\n",
               (sizeof(_shell_tests) / sizeof(_shell_tests[0])) - 1);
        return 1;
    }
