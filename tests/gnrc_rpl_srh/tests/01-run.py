...
def get_bridge(tap):
    bridge_info = subprocess.check_output(["bridge", "link"]).decode("utf-8")
    for line in bridge_info.splitlines():
        m = re.search(r"{}.+master\s+(?P<master>[^\s]+)".format(tap), line)
        if m is not None:
...
def testfunc(child):
    ...
    while True:
        exp = child.expect([r"{}\) OK \w+".format(i + 1),
                            r"OK \((\d+) tests\)"])
        if exp == 1:
            assert(int(child.match.group(1)) == i)
            break
        i += 1
