...
{
    dump_args(argc, argv);
    int afFamily = get_af_family(argv[1]);
    char *targetAddr = argv[2];
    uint16_t targetPort = atol(argv[3]);
    uint16_t localPort = atol(argv[4]);

    int err = gnrc_tcp_open_active(&tcb, afFamily, targetAddr, targetPort, localPort);
    switch (err) {
        case -EAFNOSUPPORT:
...
int gnrc_tcp_open_passive_cmd(int argc, char **argv)
{
    ...
    if (argc == 3) {
        ...
        localAddr = argv[2];
        localPort = atol(argv[3]);
    }

    int err = gnrc_tcp_open_passive(&tcb, afFamily, localAddr, localPort);
    switch (err) {
        case -EAFNOSUPPORT:
            printf("%s: returns -EAFNOSUPPORT\n", argv[0]);
            break;
...
int gnrc_tcp_send_cmd(int argc, char **argv)
{
    ...
    while (sent < toSend) {
        int ret = gnrc_tcp_send(&tcb, buffer + sent, toSend - sent, timeout);
        switch (ret) {
            case -ENOTCONN:
...
int gnrc_tcp_recv_cmd(int argc, char **argv)
{
    ...
    int timeout = atol(argv[1]);
    size_t toReceive = atol(argv[2]);
    size_t rcvd = 0;

    while (rcvd < toReceive) {
        int ret = gnrc_tcp_recv(&tcb, buffer + rcvd, toReceive - rcvd, timeout);
        switch (ret) {
            case -EAGAIN:
