...
{
    (void) event;
    ieee802154_dev_t *dev = ieee802154_hal_test_get_dev();
...
void _rx_done_handler(event_t *event)
{
     ...
     */
    int size = ieee802154_radio_indication_rx(ieee802154_hal_test_get_dev(), buffer, 127, &info);
    if (size > 0) {
...
static void _tx_finish_handler(event_t *event)
{
    ...
    assert(ieee802154_radio_confirm_transmit(ieee802154_hal_test_get_dev(), &tx_info) >= 0);

    if (!ieee802154_radio_has_irq_ack_timeout(ieee802154_hal_test_get_dev()) && !ieee802154_radio_has_frame_retrans(ieee802154_hal_test_get_dev())) {
    ...
    if (ieee802154_radio_has_frame_retrans_info(ieee802154_hal_test_get_dev())) {
...
static void _send(iolist_t *pkt)
{
    ...
    ieee802154_radio_request_set_trx_state(ieee802154_hal_test_get_dev(), IEEE802154_TRX_STATE_TX_ON);

    /* Write the packet to the radio */
    ieee802154_radio_write(ieee802154_hal_test_get_dev(), pkt);

    /* Block until the radio confirms the state change */
    while(ieee802154_radio_confirm_set_trx_state(ieee802154_hal_test_get_dev()) == -EAGAIN);

    /* Trigger the transmit and wait for the mutex unlock (TX_DONE event) */
    ieee802154_radio_request_transmit(ieee802154_hal_test_get_dev());
    ieee802154_radio_set_rx_mode(ieee802154_hal_test_get_dev(), IEEE802154_RX_WAIT_FOR_ACK);
    mutex_lock(&lock);
...
static int _init(void)
{
    ...
    ((ieee802154_dev_t*) ieee802154_hal_test_get_dev())->cb = _hal_radio_cb;
    ...
    ieee802154_radio_request_on(ieee802154_hal_test_get_dev());
    while(ieee802154_radio_confirm_on(ieee802154_hal_test_get_dev()) == -EAGAIN) {}

    current_rx_mode = IEEE802154_RX_AACK_ENABLED;
    ieee802154_radio_set_rx_mode(ieee802154_hal_test_get_dev(), current_rx_mode);

    /* Set all IEEE addresses */
    ieee802154_radio_set_hw_addr_filter(ieee802154_hal_test_get_dev(), &short_addr, &ext_addr, CONFIG_IEEE802154_DEFAULT_PANID);
    ...
    ieee802154_radio_config_phy(ieee802154_hal_test_get_dev(), &conf);
...
int _cca(int argc, char **argv)
{
    (void) argc;
    (void) argv;
    ieee802154_radio_request_cca(ieee802154_hal_test_get_dev());
    mutex_lock(&lock);
    int res = ieee802154_radio_confirm_cca(ieee802154_hal_test_get_dev());
    assert(res >= 0);
...
{
    xtimer_ticks32_t a;
    int res = ieee802154_radio_request_set_trx_state(ieee802154_hal_test_get_dev(), state);
    if (verbose) {
        a = xtimer_now();
        if(res != 0) {
            printf("%i != 0 \n", res);
    ...
    while ((res = ieee802154_radio_confirm_set_trx_state(ieee802154_hal_test_get_dev())) == -EAGAIN) {}
    if (verbose) {
        if (res != 0) {
            printf("%i != 0 \n", res);
            assert(false);
        ...
        printf("\tTransition took %li usecs\n", secs);
    }
}
...
static int rx_mode_cmd(int argc, char **argv)
{
    ...
    ieee802154_radio_set_rx_mode(ieee802154_hal_test_get_dev(), conf);
    return 0;
}
...
static int _caps_cmd(int argc, char **argv)
{
    ...
    if (ieee802154_radio_has_frame_retrans(ieee802154_hal_test_get_dev())) {
       has_frame_retrans = true; 
    }

    if (ieee802154_radio_has_auto_csma(ieee802154_hal_test_get_dev())) {
        has_auto_csma = true;
    }

    if (ieee802154_radio_has_24_ghz(ieee802154_hal_test_get_dev())) {
        has_24_ghz = true;
    }

    if (ieee802154_radio_has_sub_ghz(ieee802154_hal_test_get_dev())) {
        has_sub_ghz = true;
    }

    if (ieee802154_radio_has_irq_tx_done(ieee802154_hal_test_get_dev())) {
        has_irq_tx_done = true;
    }

    if (ieee802154_radio_has_irq_rx_start(ieee802154_hal_test_get_dev())) {
        has_irq_rx_start = true;
    }

    if (ieee802154_radio_has_irq_ack_timeout(ieee802154_hal_test_get_dev())) {
        has_irq_ack_timeout = true;
    }

    if (ieee802154_radio_has_irq_cca_done(ieee802154_hal_test_get_dev())) {
