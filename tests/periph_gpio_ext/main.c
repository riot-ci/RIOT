...
/**
 * @ingroup     tests
 * @defgroup    tests_periph_gpio_ext
 * @{
 *
 * @file
...
{
    if (strncmp(port, "ext", 3) == 0) {
        return GPIO_EXT_PORT + atoi(&port[3]);
    }
    else {
        return atoi(port);
    }
...
/* BENCHMARK_FUNC disables the interrupts and do not work on some platforms */
