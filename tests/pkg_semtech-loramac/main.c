...
{
    puts("Usage: set <dr|adr|public|net_id|tx_power> <value>");
}

static void _get_usage(void)
{
    puts("Usage: get <dr|adr|public|net_id|tx_power>");
}
...
static int _cmd_loramac_set(int argc, char **argv)
{
    ...
    else if (strcmp("net_id", argv[1]) == 0) {
        semtech_loramac_set_network_id(strtoul(argv[2], NULL, 0));
    }
...
static int _cmd_loramac_get(int argc, char **argv)
{
    ...
    if (strcmp("dr", argv[1]) == 0) {
        ...
        printf("Network ID: %lu\n", semtech_loramac_get_network_id());
    }
    else if (strcmp("tx_power", argv[1]) == 0) {
