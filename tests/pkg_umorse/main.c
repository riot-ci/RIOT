/*
 * Copyright (C) 2017 HAW Hamburg
 *
 * This file is subject to the terms and conditions of the GNU Lesser
 * General Public License v2.1. See the file LICENSE in the top level
 * directory for more details.
 */

/**
 * @ingroup     tests
 * @{
 * @file
 * @brief       Check if the uMorse package builds.
 *
 * @author      Sebastian Meiling <s@mlng.net>
 * @}
 */

#include <stdio.h>
#include <string.h>

#include "board.h"
#include "periph_conf.h"
#include "xtimer.h"
#define UMORSE_MSLEEP(m)    xtimer_usleep(m * US_PER_MS)

#include "umorse.h"
#include "print.h"

#define CODE_LEN	(64U)

#ifdef LED0_PIN
#include "periph/gpio.h"

void blink_dit(void *args, uint8_t flags)
{
    (void) args;

    gpio_set(LED0_PIN);
    UMORSE_MSLEEP(2*UMORSE_DELAY_DIT);
    gpio_clear(LED0_PIN);
}

void blink_dah(void *args, uint8_t flags)
{
    gpio_set(LED0_PIN);
    UMORSE_MSLEEP(2*UMORSE_DELAY_DAH);
    gpio_clear(LED0_PIN);
}

void blink_nil(void *args, uint8_t flags)
{
    (void) args;

    uint8_t cnt = flags & UMORSE_MASK_COUNT;
    UMORSE_MSLEEP(2 * cnt * UMORSE_DELAY_DIT);
}

const static umorse_out_t blink = {
	.dit = blink_dit,
	.dah = blink_dah,
	.nil = blink_nil,
	.params = NULL
};
#endif

const static umorse_out_t print = {
	.dit = umorse_print_dit,
	.dah = umorse_print_dah,
	.nil = umorse_print_nil,
	.params = NULL
};

const static char text[] = "Hello RIOT-OS!\nThis is uMorse.";

int test_umorse_out(void)
{
	uint8_t code[CODE_LEN];
    int ret = -1;
	printf("> Translate text of length %u into Morse code:\n\"%s\"\n",
		   (unsigned)strlen(text), text);

	memset(code, 0, CODE_LEN);
	printf("> using aligned encoding:\n");
	ret = umorse_encode_aligned(text, sizeof(text), code, sizeof(code));
    if (ret < 0) {
        return 1;
    }
	umorse_output(&print, code, ret, 0x0);
	printf("> encoded length=%d\n", ret);

	memset(code, 0, CODE_LEN);
    printf("> using compact encoding");
    #ifdef LED0_PIN
        printf(" (with LED)");
    #endif
        printf(":\n");
	ret = umorse_encode_compact(text, sizeof(text), code, sizeof(code));
    if (ret < 0) {
        return 2;
    }
	umorse_output(&print, code, ret, UMORSE_FLAG_NODELAY);
#ifdef LED0_PIN
    umorse_output(&blink, code, ret, 0x0);
#endif
	printf("> encoded length=%d\n", ret);

	return 0;
}

int main(void)
{
    puts("[START]");
    int ret = test_umorse_out();
    if (ret) {
        puts("[FAILURE]");
        return 1;
    }
    puts("[SUCCESS]");
	return 0;
}
