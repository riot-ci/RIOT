    ...
    for (const char *doc = _doc; *doc; doc++) {
        ...
        switch (r) {
            case YXML_OK:
                break;
            case YXML_ELEMSTART:
                printf("YXML_ELEMSTART \"%s\"\n", yxml.elem);
                TEST_ASSERT_EQUAL_INT(0, strcmp("foo", yxml.elem));
            ...
            case YXML_CONTENT:
                ...
                break;
            case YXML_ELEMEND:
                printf("YXML_ELEMEND content=\"%s\"\n", content);
                TEST_ASSERT_EQUAL_INT(0, strcmp("bar", content));
                break;
            default:
                printf("unhandled token %i\n", r);
        }
    }
