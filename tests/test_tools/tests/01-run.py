...
def _shellping(child, timeout=1):
    ...
    child.expect_exact('shellpong', timeout=timeout)


def _inc8(child, num, timeout=1):
    """Execute the 'inc8' command and return the value.

    HACK the 'inc8' command needs to have a marker to match the correct
    value as pyterm adds garbage around the output.

    So print 'inc8 RET' instead of only 'RET'.
    """
    child.sendline('inc8 %u' % num)
    child.expect(r'inc8 (\d+)', timeout=timeout)
    retnum = int(child.match.group(1))
    return retnum
...
def testfunc(child):
    ...
    * getting some test output without other messages
        * This one currently need hacks to work
    """
