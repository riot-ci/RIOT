 ...
 * Expected result: _nib_clear returns false and entry should still be first
 */
 ...
 * Expected result: _nib_clear returns true and entry is cleared
 */
static void test_nib_clear__empty(void)
{
    ...
    TEST_ASSERT(ipv6_addr_is_unspecified(&nib->ipv6));
    TEST_ASSERT_EQUAL_INT(0, _nib_get_if(nib));

}

/*
 * Iterates over empty NIB
 * Expected result: _nib_iter returns NULL
 */
 ...
 * Expected result: _nib_iter returns element with NULL, and with that element
 * NULL.
 *
 */
 ...
 * Expected result: _nib_iter returns element with NULL, with that element
 * another, and with the last NULL.
 */
static void test_nib_iter__two_elem(void)
{
    _nib_t *nib1, *nib2, *res;
    ipv6_addr_t addr = { .u64 = { { .u8 = GLOBAL_PREFIX },
                                { .u64 = TEST_UINT64 } } };
 ...
 * Expected result: _nib_get() returns NULL
 */
 ...
 * Expected result: nib->info flags set to NUD_STATE_REACHABLE and NIB's event
...
static void test_nib_drl_add__success(void)
{
    ...
    TEST_ASSERT(nib_dr->next_hop == _nib_iter(NULL));
    TEST_ASSERT(nib_dr->next_hop->mode & (_DRL));
    TEST_ASSERT(ipv6_addr_equal(&addr, &nib_dr->next_hop->ipv6));
    TEST_ASSERT_EQUAL_INT(IFACE, _nib_get_if(nib_dr->next_hop));
}

/*
 * Creates a default router list entry, sets another flag, and tries to remove it.
 ...
 * Expected result: _nib_drl_iter returns element with NULL, and with that element
 * NULL.
 *
 */
static void test_nib_drl_iter__one_elem(void)
{
    ...
    TEST_ASSERT_NOT_NULL((nib = _nib_drl_add(&addr, IFACE)));
...
{
    _nib_dr_t *nib1, *nib2, *res;
    ipv6_addr_t addr = { .u64 = { { .u8 = GLOBAL_PREFIX },
                                { .u64 = TEST_UINT64 } } };

    TEST_ASSERT_NOT_NULL((nib1 = _nib_drl_add(&addr, IFACE)));
    addr.u64[1].u64++;
...
static void test_nib_drl_iter__three_elem(void)
{
    ...
    TEST_ASSERT_NOT_NULL((nib2 = _nib_drl_add(&addr, IFACE)));
    addr.u64[1].u64++;
    TEST_ASSERT_NOT_NULL((nib3 = _nib_drl_add(&addr, IFACE)));
    TEST_ASSERT_NOT_NULL((res = _nib_drl_iter(NULL)));
    TEST_ASSERT(res == nib1);
    TEST_ASSERT_NOT_NULL((res = _nib_drl_iter(res)));
    TEST_ASSERT(res == nib2);
...
static void test_nib_drl_iter__three_elem_middle_removed(void)
{
    ...
    addr.u64[1].u64++;
    TEST_ASSERT_NOT_NULL((nib3 = _nib_drl_add(&addr, IFACE)));
    nib2->next_hop->mode = _EMPTY;
    _nib_drl_remove(nib2);
    TEST_ASSERT_NOT_NULL((res = _nib_drl_iter(NULL)));
    TEST_ASSERT(res == nib1);
    TEST_ASSERT_NOT_NULL((res = _nib_drl_iter(res)));
    TEST_ASSERT(res == nib3);
...
{
    _nib_dr_t *nib, *nib_res;
    ipv6_addr_t addr = { .u64 = { { .u8 = GLOBAL_PREFIX },
                                { .u64 = TEST_UINT64 } } };

    TEST_ASSERT_NOT_NULL((nib = _nib_drl_add(&addr, IFACE)));
    nib->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_UNREACHABLE;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib);
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib);
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib);
...
static void test_nib_drl_get_dr__round_robin2(void)
{
    ...
    TEST_ASSERT_NOT_NULL((nib2 = _nib_drl_add(&addr, IFACE)));
    nib2->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_INCOMPLETE;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib1);
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
...
static void test_nib_drl_get_dr__success1(void)
{
    ...
    TEST_ASSERT_NOT_NULL((nib1 = _nib_drl_add(&addr, IFACE)));
    addr.u64[1].u64++;
    nib1->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_STALE;
    TEST_ASSERT_NOT_NULL((nib2 = _nib_drl_add(&addr, IFACE)));
    nib2->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_INCOMPLETE;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib1);
...
/*
 * Tries to get the default router from a list of two routers
 * the first unreachable, the second reachable
 * Expected result: _nib_drl_get_dr() always returns the second router
 */
...
{
    _nib_dr_t *nib1, *nib2, *nib3, *nib_res;
    ipv6_addr_t addr = { .u64 = { { .u8 = GLOBAL_PREFIX },
                                { .u64 = TEST_UINT64 } } };
    ...
    nib2->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_INCOMPLETE;
    TEST_ASSERT_NOT_NULL((nib3 = _nib_drl_add(&addr, IFACE)));
    nib3->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_DELAY;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib3);
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib3);
}
...
static void test_nib_drl_get_dr__success4(void)
{
    ...
    nib1->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_UNREACHABLE;
    TEST_ASSERT_NOT_NULL((nib2 = _nib_drl_add(&addr, IFACE)));
    nib2->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_REACHABLE;
    TEST_ASSERT_NOT_NULL((nib3 = _nib_drl_add(&addr, IFACE)));
    nib3->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_INCOMPLETE;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib2);
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res == nib2);
    nib2->next_hop->info = GNRC_IPV6_NIB_NC_INFO_NUD_STATE_UNREACHABLE;
    TEST_ASSERT_NOT_NULL((nib_res = _nib_drl_get_dr()));
    TEST_ASSERT(nib_res != nib2);
}
 ...
 */
static void test_nib_drl_get_dr__success5(void)
{
    _nib_dr_t *nib1, *nib2, *nib3, *nib_res;
    ipv6_addr_t addr = { .u64 = { { .u8 = GLOBAL_PREFIX },
                                { .u64 = TEST_UINT64 } } };
...
Test *tests_gnrc_ipv6_nib_internal_tests(void)
{
    EMB_UNIT_TESTFIXTURES(fixtures) {
        ...
        new_TestFixture(test_nib_drl_get_dr__success5),
